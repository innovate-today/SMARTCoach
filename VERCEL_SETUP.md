...
Use **Copy Coach Invite** from the Customer Links section after launch validation is complete. It will not copy until the live smoke-test checklist is complete and the launch sign-off is stamped. It creates a short coach-facing note with the SMARTCoach link, account key, and coach access-code instructions for the purchased plan, including where to enter the account key and coach code on the phone app. Essential invites also tell the coach where to enter the account key for stopwatch-only access. The copied time and post-launch follow-up checks are saved locally per account, update the follow-up summary, and appear in later activation records.
