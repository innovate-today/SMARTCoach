...
Use **Copy Activation Record** after the customer passes setup. It creates a plain support-note summary with the account key, plan, coach seats, subscription, Stripe IDs, registry/setup/access state, recent automation update, live smoke-test progress, launch sign-off, coach-invite reminder or copied time, next action, and customer link. The next action will continue pointing to the live smoke test, launch sign-off, activation record, or coach invite until each step is complete.

Use **Copy Coach Invite** from the Customer Links section after launch validation is complete. It will not copy until the live smoke-test checklist is complete and the launch sign-off is stamped. It creates a short coach-facing note with the SMARTCoach link, account key, and coach access-code instructions for the purchased plan, including where to enter the account key and coach code on the phone app. Essential invites also tell the coach where to enter the account key for stopwatch-only access. The copied time is saved locally per account and appears in later activation records.
...
- **Support handoff:** **Copy Activation Record** includes the account key, subscription, Stripe IDs when available, setup/access state, smoke progress, launch sign-off, coach-invite reminder or copied time, next action, customer link, status link, and coach page validation links.
