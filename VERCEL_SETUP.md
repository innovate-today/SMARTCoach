...
For the current launch path, configure production with `/onboarding.html` and customer account storage. New customer accounts should be saved to customer account storage instead of adding a new set of Vercel variables for every coach.

At minimum, production should have:

- customer account storage values, such as `KV_REST_API_URL` and `KV_REST_API_TOKEN`, or `SMARTCOACH_REGISTRY_REST_URL` and `SMARTCOACH_REGISTRY_REST_TOKEN`
...
Bug Trak webhook payloads include flat GHL-friendly fields such as `bugSummary`, `bugDetails`, `bugExpected`, `bugUrgency`, `bugArea`, `bugPage`, `bugPageTitle`, `bugCoachName`, `bugCoachEmail`, and `bugAccountKey`, plus the original nested `report` object for compatibility.
...
For new customers, save these account values through `/onboarding.html` -> **Save Account Setup**. SMART Trak uses the saved customer account record at runtime before falling back to account-specific Vercel variables.
...
Use **Check Customer Access** in the **Live Smoke Test** section after saving a customer account. It calls the live account status endpoint for that account key and shows whether setup, subscription access, customer account storage, and coach access-code requirements are ready before opening every coach page manually. A warning state means the account is ready but the current browser or phone still needs a coach access code. Use **Test Access Rules** to run no-save checks that active/trialing allow access while past due, paused, unpaid, canceled, incomplete, and incomplete-expired statuses block access. The live smoke-test summary should say complete before the coach invite is sent, and **Copy Smoke Status** can capture the current pass/missing checklist, launch sign-off, activation-record copy status, final activation-record copy status, coach invite, post-launch follow-up, and next action for support notes.
...
If the setup form and account lookup show different account keys, `/onboarding.html` warns and blocks saving, setup previews, and live access checks until they match. This prevents validating or saving the wrong coach account during activation.

After generating or saving a customer account, use the **Activation Runbook** panel for the exact support order: Check System, Copy System Status, Test Setup First, Save Account Setup, add the single SMART Trak custom link, verify coach access, complete the live smoke test, stamp and save the activation handoff, send the coach invite, then confirm first login, first sync, and phone bulk archive. Check System should show no launch blockers for the automation secret, setup code, customer account storage, Stripe webhook, coach session secret, coach access enforcement, and parent email rollout gate.
...
The `/onboarding.html` page also shows these account storage field names in **Customer Account Storage Setup** so internal support can copy the names while setting up Vercel.
...
8. Save the account with **Save Account Setup**.
...
Use `/live-launch-validation.html?account=<account-key>` for the support-facing live checklist, fill-in validation record, issue log, stop/go rules, and handoff checks. The checklist saves progress separately per account key, shows the active account scope, can reopen `/onboarding.html` for the same account, includes account-scoped Stopwatch, Dashboard, Account Status, and coach-page validation links, and has **Copy Validation Link** plus **Copy Summary** for saving paste-ready support notes. The markdown source is also available in `LIVE_LAUNCH_VALIDATION.md`.
...
- **GHL subscription automation:** the GHL Subscription Payload updates customer account storage and appears in recent account update history, while private tokens and coach access codes remain in **Save Account Setup**.
