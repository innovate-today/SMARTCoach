...
The `/onboarding.html` page also includes **Launch Security Values** with copy-ready field names and safe value notes for `SMARTCOACH_ADMIN_SETUP_CODE`, `SMARTCOACH_AUTOMATION_SECRET`, `SMARTCOACH_STRIPE_WEBHOOK_SECRET`, `SMARTCOACH_SESSION_SECRET`, `SMARTCOACH_REQUIRE_COACH_ACCESS`, and the parent-email rollout flag. Use **Generate Launch Secrets** there to create separate setup, automation, and session secret values, then **Copy Security Values** to paste the generated values into Vercel before running **Check System** for launch readiness.
