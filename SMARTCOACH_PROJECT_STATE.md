...
- Miles Trak API record dedupe now keys first by `source_record_id` before object record id when combining live SMART Trak performance records with the account training mirror. This prevents the same synced workout from being counted once from GHL and once from the mirror when both copies exist with different object ids, which likely explains Hayden showing 231 miles instead of the hand-summed 213 miles; the overage was 18.0 miles, matching the Jul 13 week total. Coach how-to and regression coverage were updated.
...
552. Miles Trak API dedupe now uses `source_record_id` before object record id when merging live performance records with the account training mirror. This keeps the same synced workout from counting twice when both copies are present with different object ids. The issue matched Hayden's reported 231 vs 213 mismatch: the 18.0-mile overage equals the Jul 13 weekly total. Coach how-to and regression coverage were updated.
