...
- Remaining Speed Metrics dashboard work: training-load/volume counting still needs wiring. The coach rule is now set: speed volume is only timed sprint/runway distance, such as `5 x 30m = 150m`, warmup/cooldown are not included, and one saved speed session should count as a completed workout for every athlete with at least one saved rep.
