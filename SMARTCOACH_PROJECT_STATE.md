...
- SMARTCoach app timed quality-session targets now stay timed in the athlete Details / Multi Athlete Timer target view. A Training Calendar workout such as `12 x 45 sec @ Threshold` now displays `Today: 12 x 45 sec` with a `00:45.0` timed target instead of falling back to the Threshold default distance target (`1000m`). Coach how-to and regression coverage were updated.
