...
277. Athletic.net Meet History Import also supports the full-season matrix layout with athlete rows and meet-date columns. The importer cross-references the Meet List to convert date columns into meet names/dates, reads Race Distances to map subscripted result cells to events such as 2 Mile or 5000m, skips Athletic.net initials/avatar columns, and creates one historical result per filled cell.
