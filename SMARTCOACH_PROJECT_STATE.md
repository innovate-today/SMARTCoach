...
496. Dashboard header actions now render as one compact horizontal toolbar instead of stacked action rows. Header buttons use slightly smaller sizing and horizontal overflow as the narrow-screen fallback, keeping visible shortcuts such as Dashboard, Log Miles, Miles Trak, Speed Trak, Log Race Result, Manage Meets, Field Practice, Keep Trak, and Weather on one row when space allows.
