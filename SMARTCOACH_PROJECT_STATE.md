...
187. Adjusted SMARTCoach Pro Mobile App hooked runner rows so the hook pill no longer squeezes the athlete name. Hook labels move below the name lane, and active timing rows hide the unused info/delete controls while timing so the athlete name and End/Stop buttons stay on one compact row.
