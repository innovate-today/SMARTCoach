...
355. **Email Calendar Links** now includes an **Email Provider** selector with Gmail, Outlook, Yahoo Mail, and Default mail app. Gmail is selected by default so coaches working inside the GHL embed open a real web compose window instead of relying on Chrome's blocked `mailto:` handler. Default mail app still uses `mailto:` for coaches with a local/default mail app configured. Tests cover the provider URLs, the Gmail default, and keep `mailto:` from forcing a blank tab.
