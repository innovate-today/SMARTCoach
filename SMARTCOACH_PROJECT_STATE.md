...
- Dashboard Completed Workouts **Correct Training Entry** now uses a completed-volume amount field plus a unit selector for miles, kilometers, or meters. Saving a correction sends a unit-bearing value such as `1.01 mi` instead of saving ambiguous text like `1.01`, so dashboard volume calculations have the unit coaches intended. Dashboard Log Miles Easy Miles now uses the same amount-plus-unit pattern before saving manual mileage. Coach how-to and regression coverage were updated.
...
550. Dashboard Completed Workouts **Correct Training Entry** and Log Miles Easy Miles now make coaches choose the completed-volume unit. Correction saves now combine the amount with miles, kilometers, or meters before sending the API payload, so rows save as values like `1.01 mi` instead of ambiguous unitless values like `1.01`. The modal controls, local save payload, coach how-to guide, and regression coverage were updated.
