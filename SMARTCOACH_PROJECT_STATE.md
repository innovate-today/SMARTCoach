...
283. Meet History now has a Sport filter with Track and Cross Country support, carries sport into meet groups, shows Sport as a sortable result column, and displays sport in meet metadata so imported history can be separated even when season labels are just years or unlisted. Untagged older rows recover sport from Athletic.net import notes and event names so filtering does not make historical results disappear as Unlisted. Added sport-filter wiring coverage in `tests/run-all.js`; `npm test` passed on 2026-06-04.
