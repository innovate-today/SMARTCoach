...
- SMART Trak now records lightweight API usage by account for the unified `/api/smart-trak/*` routes. The account registry stores hourly buckets with method, route, status, count, and handler duration, batched through the registry pipeline without saving request bodies, athlete details, or private roster content. The Dashboard has an admin-only **API Usage** modal for head coaches/support to review total calls, average time, errors, top routes, status codes, and recent hourly buckets for today/the last 7 days. Coach how-to and regression coverage were updated.
