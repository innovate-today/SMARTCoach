...
- SMARTCoach app Equipment Trak now shows only active roster athletes in the phone issue list once roster data is loaded, and adds **Athlete Order** with Name A-Z, Girls/Boys/Name, and Grade/Name options. The selected order is saved per account on the device. Sorted issue cards still save back to the correct athlete records by original runner index. Coach how-to and regression coverage were updated.
...
555. SMARTCoach app Equipment Trak issue list now filters out inactive roster athletes and lets coaches choose the issue-day order: Name A-Z, Girls/Boys/Name, or Grade/Name. The order preference is stored per account on the device, and sorted cards save back to the correct athlete records. The coach how-to guide and regression coverage were updated.
