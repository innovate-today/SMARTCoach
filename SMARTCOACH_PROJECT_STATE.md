...
403. Adjusted the Dashboard summary layout so the three card groups each occupy a single row on desktop: Training Load keeps its five cards together, Roster Summary gets its own row, and Meet Results Summary gets its own row. The Dashboard max width was widened to reduce wrapping in the embedded view.
