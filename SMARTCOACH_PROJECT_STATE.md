...
- Copy Smoke Status now also includes coach invite copied status, post-launch follow-up progress, and the current next action, so the support note can cover the full launch state.
...
- Copy Activation Record now requires the live smoke checklist to be complete and launch sign-off to be stamped; support should use Copy Smoke Status for partial launch notes.
