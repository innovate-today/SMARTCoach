...
- Strava athlete connection confirmation follow-up: `/strava-connected.html` no longer treats a bare `?strava=connected` URL as proof that an athlete connection saved. The OAuth callback now appends `stravaConfirmed=1` only after `saveAccountRecord` succeeds, and the static success page plus returned-message helpers only show connected language for that backend-confirmed redirect. Old/manual `strava=connected` URLs now tell the athlete/coach to return to SMARTCoach and verify instead of falsely saying the athlete is connected. Regression coverage was updated. Coach how-to and What's New were not updated because Strava remains a private beta/admin-only feature.
- Strava athlete connection follow-up: the Training Calendar Strava modal now refreshes saved athlete connection status after a connect link is copied/opened and whenever the coach returns focus to the tab. The backend callback already saved the athlete token before redirecting to `/strava-connected.html`; the bug was that the original Training Calendar modal stayed on its pre-OAuth local state and could continue showing `Not connected` until the modal/page was reopened. Regression coverage was updated. Coach how-to and What's New were not updated because Strava remains a private beta/admin-only feature.
- Strava athlete import follow-up after Vanessa reconnect: production showed the updated `read,activity:read,activity:read_all` scope but Strava still returned zero recent activities for the selected athlete. The athlete-activities API now returns safe diagnostic metadata for empty loads (raw Strava row count, sanitized returned count, page/limit, after/before ISO window), and the Training Calendar Strava modal displays those diagnostics plus the connected Strava athlete id/name when no activities are returned. This should distinguish a true Strava zero-row response from SMARTCoach filtering or a connected duplicate Strava account. Regression coverage was updated. Coach how-to and What's New were not updated because Strava remains a private beta/admin-only feature.
- Strava athlete import follow-up: Strava athlete connect links now request `activity:read_all` in addition to the previous `read,activity:read` scopes. The Training Calendar Strava modal now detects older connected athlete tokens missing that private-activity scope and tells the owner/admin to reconnect the athlete instead of leaving a blank recent-activities result ambiguous. This was traced from the modal's selected athlete/status, saved connection scope, athlete-activities API call, and Strava list-activities behavior; existing athlete tokens need a fresh reconnect to gain the new scope. Regression coverage was updated. Coach how-to and What's New were not updated because Strava remains a private beta/admin-only feature.
...
- Training Calendar Strava beta activity loading now asks Strava for up to 30 activities from the last 45 days for the selected athlete instead of only the first 10 latest rows. If the selected athlete is connected but Strava returns no activities, the modal now says that Strava returned no recent activities and reminds the owner/admin that the workout must sync to that athlete's Strava account before SMARTCoach can import it. The athlete-activities API accepts optional `after`/`before` Strava query bounds. Regression coverage was updated. Coach how-to and What's New were not updated because Strava remains a private beta/admin-only feature.
...
- Staff Access invite copy was cleaned up so **Create Invite** / **Copy Invite** messages include one SMART Trak desktop link instead of both the normal dashboard link and the private invite link. Full Access invite messages now say `Use this private link to open SMART Trak:` and use the invite-token URL; personal code-only messages still include the normal SMART Trak link. Coach how-to and regression coverage were updated. What's New is pending user approval.
...
- SMART Trak Training Calendar Strava athlete connections now guard against a connected Strava account being shown or used under the wrong SMART Trak athlete. The modal rejects obvious selected-athlete/name mismatches such as one athlete row showing another athlete's Strava account, activity/detail requests send the full selected athlete identity, and the backend will not use a mismatched saved connection for imports. If the selected athlete name exactly matches the saved Strava account, the backend can repair the saved athlete mapping and remove the stale duplicate for that Strava account. Coach how-to and regression coverage were updated. What's New is pending user approval.
- Follow-up to Dashboard Completed Workouts rep-volume corrections: dashboard render and dashboard API now let an explicit corrected completed volume such as `4 x 400 m` win before deriving volume from split count. This keeps target-review split rows from forcing the Volume column back to `8 x 400m` or another stale inferred value after a coach corrects completed volume. Regression coverage was updated. Coach how-to was not changed because the coach workflow remains the same, and What's New was not updated pending user approval.
- Dashboard Completed Workouts correction modal now preserves rep-style completed volume typed without an inline unit, such as `4 x 400` with the **Meters** dropdown selected. Previously the UI payload builder reduced that to the first number only (`4 m`), so the desktop row could keep showing an incorrect low volume after save. Regression coverage was updated. Coach how-to was not changed because the workflow instruction remains the same, and What's New was not updated pending user approval.
- Follow-up to Dashboard Completed Workouts correction saves: the correction API now also retries without optional mapped `season_year` when an account field setup rejects it, matching the existing sync-session fallback and fixing `We couldn't validate the mapped field: season_year` from the correction modal. Regression coverage was updated. What's New was not updated per user instruction.
- Dashboard Completed Workouts correction saves now translate the coach-visible **Cross Country** / **Track** season choice into the saved sport values SMART Trak accepts, and retry without the optional mapped `sport` field if an older account field setup rejects it. This fixes the correction modal error `We couldn't validate the mapped field: sport` when editing rows such as an `8 x 400m` completed workout. Regression coverage was updated. What's New was not updated per user instruction.
- SMARTCoach Pro app workout sync now treats confirmed saved workout rows as saved even when dashboard mirror confirmation is delayed, so the phone app no longer tells coaches to retry after SMART Trak has already accepted records. The sync backend also checks for an existing `source_record_id` immediately before create, re-checks after transient/rate-limit-style create errors, and writes each saved performance record to the training mirror as soon as it gets a record ID. This narrows the duplicate window when a sync partially succeeds and then returns a "too much data"/rate/attempt error. Coach how-to and regression coverage were updated.
- SMART Trak Training Calendar Strava beta now has the first coach/athlete connection flow. On beta accounts, coaches can open **Strava**, choose a SMART Trak athlete, copy or open that athlete's Strava connect link, and then load/import that athlete's recent Strava activities. The existing owner/admin account-level Strava test routes remain intact, but athlete imports use athlete-specific tokens saved under `stravaAthleteConnections` in the account record; sanitized connection status hides tokens in the UI/API. Athlete OAuth links return to `/strava-connected.html`, a simple no-session success/error page. The athlete OAuth state is valid for 7 days, while the older owner/admin test state remains short-lived. Coach how-to and regression coverage were updated. What's New is pending user approval.
- Dashboard Speed Trak corrupted-distance cleanup: Speed Metrics rows now prefer an explicit sprint distance from labels such as `30m Start` when a saved per-rep meters value is clearly corrupted, such as `650m` on a 30m start. Displayed volume and velocity use the corrected explicit distance, and the sprint correction save path writes the cleaned distance back on the next save. Regression coverage was updated. What's New and coach how-to were not updated because this fixes bad data/rendering caused by the earlier sprint correction bug.
- Dashboard Speed Trak Target Review label cleanup: Speed Metrics rows now preserve the saved speed rep labels in Target Review (`Rep 1` through `Rep 8`) instead of applying the normal rep/rest pairing label logic that made 8 speed reps display as two each of `Rep 1` through `Rep 4`. Regression coverage was updated. What's New and coach how-to were not updated because this is a display correction.
- Dashboard Speed Trak correction cleanup: sprint corrections no longer copy the multi-line correction Notes text onto every individual speed rep. Speed rep Target Review chips now use only the actual timed rep result details, so old correction/audit lines such as update timestamps do not render as fake Lap chips like `Lap 88`. Regression coverage was updated. What's New and coach how-to were not updated because this fixes a broken display/save side effect.
- Dashboard Speed Trak correction follow-up: Speed Metrics rows now carry explicit Track season context for dashboard season filters, so coaches do not need to edit a Speed Trak row just to make it appear under `Summer 2026`. The sprint correction path now ignores aggregate completed-volume labels such as `130m completed` when multiple speed reps are present, preventing an unchanged season-only correction from rewriting every rep to 130m and inflating the row to `4 x 130m completed`. Regression coverage was updated. What's New was not updated per user instruction.
- Dashboard season filtering follow-up: `Summer 2026` and other track date-bucket seasons now require explicit Track sport context instead of accepting every unclassified row by date, so Cross Country mileage is not pulled into Summer just because it happened in June/July. Training Calendar Log Miles was brought in line with Dashboard Log Miles: it now requires a Season choice and a distance unit, and manual-mileage API rejects ambiguous saves that omit Cross Country/Track or a distance unit. Regression coverage was updated. What's New was not updated per user instruction.
- Dashboard season filtering follow-up: the roster summary/table now narrows to athletes with completed volume in the selected season/activity view instead of counting every active roster athlete when a season such as `Cross Country 2026` is selected. Completed workout corrections now include a **Season** selector and save sport/season/year on the performance record and training mirror, so older rows that were saved under the wrong season (for example `Summer 2026`) can be corrected to `Cross Country 2026` without adding month-based guessing back into the dashboard. Coach how-to and regression coverage were updated. What's New was not updated per user instruction.
- Dashboard season filtering was corrected after the Aug 1 follow-up overclassified summer manual/track mileage as Cross Country. The Dashboard no longer infers Cross Country from month or from the athlete's current roster/season identity; completed workout rows use the sport/season saved on the row, so `Cross Country 2026`, `Track 2027`, and `Cross Country 2027` remain separate historical buckets as athletes move between seasons. Log Miles now requires the coach to choose Cross Country or Track before saving; Cross Country entries save as `Cross Country <year>`, while Track entries save with the track calendar season for the workout date. Manual-mileage API fallback no longer silently defaults missing sport to Cross Country. Coach how-to and regression coverage were updated. What's New is pending user approval.
- Dashboard Cross Country season filtering was tightened again after the Aug 1 dashboard showed only 77 workouts under `Cross Country 2026`: non-track training rows from May through November now resolve to the cross country season year, so May cross country mileage is not left behind in `Spring 2026`. Track rows remain in calendar seasons (`Winter 2026`, `Spring 2026`, `Summer 2026`) based on date. Coach how-to and regression coverage were updated. What's New is pending user approval.
- Follow-up to Dashboard season filtering: summer/fall non-track training rows now resolve to the cross country season year even when older saved rows do not explicitly say CC/XC/Cross Country. This keeps summer cross country mileage/workouts under `Cross Country 2026` instead of creating a `Summer 2026` cross country bucket, while explicit Track rows still remain in the normal date bucket such as `Summer 2026`. Dashboard season choices, current-season filtering, and reload/render all use the same row season key; saved records are not rewritten. Coach how-to and regression coverage were updated. What's New is pending user approval.
- Dashboard season filtering now treats Cross Country as its own sport season while leaving Track in the date-based season bucket. Current-season filtering now matches the active current keys instead of every row in the current year: late-summer/fall Cross Country rows resolve to `Cross Country 2026`, while July Track rows remain `Summer 2026`. The dashboard also recognizes older rows as Cross Country when the saved row/group/workout text clearly says CC/XC/Cross Country, without rewriting saved records. Coach how-to and regression coverage were updated. What's New is pending user approval.
- Dashboard Completed Workouts Target Review now derives a display-only current-fitness pace target for completed Easy/Recovery Run and Long Run mileage rows that have no saved planned target. Manual mileage, synced entries, and non-calendar completed mileage rows can now show actual pace versus the expected current-fitness pace instead of falling to `No target saved`, while saved targets, saved target differences, split/rep reviews, and Actual Rep Paces still take priority. Correction notes cleanup stays intact because the derived target is not written back into editable Notes. Coach how-to and regression coverage were updated. What's New was not updated per user instruction.
- SMART Trak roster loading now excludes contacts tagged `smartcoach-prospect` from athlete roster, Dashboard roster/count, and Athlete Calendar lookup paths. This keeps internal coach-prospect outreach contacts and the main SMARTCoach Pro admin sub-account Coach Info fields out of test-team/athlete views even when those contacts have names or custom field data. Regression coverage was updated. Coach how-to and What's New were not updated because this is internal campaign/contact hygiene.
- Dashboard Completed Workouts **Correct Training Entry** now treats Strava import/audit lines as system metadata instead of editable Notes. Strava activity id, matched workout/group, activity summary, lap-exposure diagnostics, and import status lines stay saved in the record for troubleshooting but no longer preload into the Notes textarea or duplicate when a correction is saved. `Rep pace details:` still loads into **Actual Rep Paces**. Regression coverage was updated. Coach how-to and What's New were not updated because this remains owner/admin Strava beta cleanup.
- SMARTCoach app Equipment Trak now shows only active roster athletes in the phone issue list once roster data is loaded, and adds **Athlete Order** with Name A-Z, Girls/Boys/Name, and Grade/Name options. The selected order is saved per account on the device. Sorted issue cards still save back to the correct athlete records by original runner index. Follow-up bumped the mobile app version/build stamp after the phone still showed the old Equipment screen, then renamed the Equipment modal button to **Refresh Data** because that control reloads equipment records but does not reload the app shell. Coach how-to and regression coverage were updated. What's New was not updated per user instruction.
- Dashboard Completed Workouts **Correct Training Entry** now keeps generated target-review/sync details out of the editable **Notes** field. The modal and correction API both strip system labels such as Planned volume, Actual, Difference, Current fitness, and Workout Location, and they normalize saved literal `\n` sequences before filtering so older repeated-note text can be cleaned on the next correction save. Regression coverage was updated. Coach how-to and What's New were not updated.
- Follow-up to the Hayden Miles Trak mismatch: the exported training CSV showed saved completed-volume text such as `.19 mi` being parsed as `19 mi`, and unitless `.78` being parsed as zero. Dashboard API and browser fallback completed-volume parsing now accept leading-decimal distances, avoid matching a unit-bearing number inside another decimal or time-like value, and keep correction-modal preload aligned. Coach how-to and regression coverage were updated. What's New was not updated.
- Follow-up to the Miles Trak Hayden total mismatch: the previous broad `source_record_id` dedupe dropped legitimate live rows that shared an older source id, moving Hayden from 231/too high to 208.6/too low. The dashboard API now preserves all live SMART Trak performance records by object id and only filters training-mirror backup rows when their `source_record_id` already exists in live records. This keeps live duplicate/manual rows intact while preventing the same live+mirror copy from double-counting. Coach how-to and regression coverage were updated.
- Dashboard old completed-workout rows with unitless completed volume now display that value as miles and open **Correct Training Entry** with Miles selected. This gives coaches a practical cleanup path for records that were already saved as values like `4`: opening and saving the correction writes the unit-bearing value back as `4 mi`. New blank Log Miles and correction entries still start on **Select unit** and block saves until a unit is chosen. Coach how-to and regression coverage were updated; What's New remains unchanged.
- Dashboard Completed Workouts **Correct Training Entry** and Dashboard Log Miles Easy Miles now require coaches to choose a completed-volume unit before saving. Both controls start on **Select unit**, block unitless saves, and send unit-bearing values such as `1.01 mi` instead of ambiguous text like `1.01`, so dashboard volume calculations have the unit coaches intended. Log Miles field-info bubbles now use the fixed-position tooltip helper so the help bubble stays on screen. Coach how-to and regression coverage were updated. What's New was intentionally left unchanged per user instruction.
...
- SMART Trak Training Calendar Strava beta now includes an owner/admin activity-detail inspector. Each recent Strava activity can be inspected before import; the inspector calls a new beta-gated Strava detail endpoint and reports how many laps, metric splits, standard splits, and stream points Strava actually exposes for that activity. This is the diagnostic step before rep breakout because Strava may not provide every Garmin interval through its API. Owner/admin beta only; coach guide unchanged.
- SMARTCoach Admin Strava Test now includes a first owner/admin import path: after loading recent Strava activities, the owner/admin can choose an active SMART Trak athlete and import one Strava activity as a completed Easy/Recovery workout. The import uses the existing `/api/smart-trak/sync-session` completed-workout path so contact matching, duplicate checks, training mirror writes, and Completed Workouts rendering stay consistent. `sync-session` now accepts an optional `sourceSessionId`, and Strava imports pass `strava_<activity id>` so duplicate protection keys to the actual Strava activity instead of only the date/group. This remains out of the coach how-to guide and What's New because it is owner/admin test tooling only.
- Follow-up to the Strava owner/admin connect flow: clicking **Connect Strava** now starts OAuth through a session-authenticated fetch that returns the Strava authorization URL, then navigates the browser to Strava. The previous full-page `/strava-start` navigation could not send the signed SMART Trak coach-session header, so production showed raw JSON saying the coach access code was required even though the dashboard was unlocked. Regression coverage was updated; this remains owner/admin-only and is not in the coach how-to guide.
- Follow-up to the Strava owner/admin test diagnostics: the Strava Test modal now preserves backend status errors after rendering fallback cards. This prevents an invalid account/admin context, such as a URL still using `account=YOUR_ACCOUNT_KEY`, from being overwritten by the generic "Strava env vars are missing" message. Admin-only regression coverage was updated; this remains out of the coach how-to guide.
- Follow-up to the Strava owner/admin test path: the Strava Test modal now shows safe production diagnostics for the three required env vars (`STRAVA_CLIENT_ID`, `STRAVA_CLIENT_SECRET`, and `STRAVA_REDIRECT_URI`) as present/missing booleans, plus the Vercel runtime label/commit. No secret values are exposed. This is to trace why production can show the Strava Test UI while still reporting Strava env vars missing. Regression coverage was updated.
- SMART Trak now has an owner/admin-only Strava Test path for the first Strava integration validation. Dashboard Admin shows **Strava Test** with connection status, a Connect Strava OAuth button, and a recent-activities read test. The backend uses the existing unified `/api/smart-trak/[route]` function for `strava-status`, `strava-start`, `strava-callback`, and `strava-activities`; `/api/strava/oauth/callback` rewrites to the existing callback route so the confirmed Strava callback domain works without adding another Vercel function. Tokens are stored on the account registry record under `stravaConnection` and are never shown in the UI. This remains out of the coach how-to guide and What's New because it is owner/admin test tooling only. Regression coverage was added.
- SMARTCoach app target display now uses a target-mode resolver before calculating workout targets. The renderer classifies selected calendar workouts as Easy, Quality, Speed, Race, Off, or legacy from structured day fields first; Easy/Long Run targets now go directly to mile-pace rules from current fitness, Race/Off skip pace targets, and unknown/blank legacy text no longer silently creates the generic `400m` / `75-85%` target. Text parsing remains for quality/speed details and old saved workouts. Coach how-to and regression coverage were updated.
- Follow-up to the SMARTCoach app easy-run target fix: target math now infers Easy/Recovery or Long Run from a calendar workout's title/day type when the phone has only a thin saved selection such as `Easy run · SMART Trak Calendar - Group 2-CC`. This keeps title-only easy calendar selections from falling to the generic `400m` / `75-85%` target while still keeping custom quality-session titles out of target calculations. Regression coverage was updated.
- SMARTCoach app target display now treats saved Easy/Recovery calendar workout text such as `6 mi easy` as an Easy/Recovery mile-pace target even if the separate workout type field is missing during reload. Easy/Recovery and Long Run targets are also protected from rep-distance fallback, so notes or strides cannot turn an easy day into a generic `400m` target at `75-85%`. Coach how-to and regression coverage were updated.
- SMART Trak now records lightweight API usage by account for the unified `/api/smart-trak/*` routes. The account registry stores hourly buckets with method, route, status, count, and handler duration, batched through the registry pipeline without saving request bodies, athlete details, or private roster content. SMARTCoach Admin can open an admin-only **API Usage** modal to review total calls, average time, errors, top routes, status codes, and recent hourly buckets for today/the last 7 days. This is intentionally not in the coach how-to guide or What's New. Regression coverage was updated.
- Dashboard Completed Workouts Target Review now shows a compact **Actual vs Target** table when coaches enter Actual Rep Paces for pace-based targets such as `12 x 45 sec @ Threshold`. The table compares each work rep pace against the saved target pace range and shows fast/on/slow differences; recovery paces remain saved for review without being graded against the work target. Timed rep split durations like `00:45` are no longer colored as outside target when the planned target is a `/mi` pace range. Coach how-to and regression coverage were updated.
- SMARTCoach app group refresh now lets inactive roster status supersede a saved local phone group row. When a SMART Trak training group reloads, the phone app rebuilds visible timing rows from active group athletes and only preserves saved local runner history when that runner still matches the active roster for the group. This keeps an inactive athlete from reappearing in the phone app just because they had been saved in the group on that phone. Coach how-to and regression coverage were updated.
- Dashboard Completed Workouts **Correct Training Entry** now includes **Actual Rep Paces** for watch-reported pace entry on timed workouts. Coaches can enter lines such as `Rep 1 work pace: 6:15 /mi` and `Rep 1 recovery pace: 8:42 /mi`; the paces are saved to the SMART Trak note, reload into the correction modal, and display in Target Review. Work paces compare against the saved target range with fast/on/slow chips; recovery paces are saved and shown without target comparison. Coach how-to and regression coverage were updated.
- SMARTCoach app timed quality-session targets now calculate the pace for the timed work rep in the athlete Details / Multi Athlete Timer target view. A Training Calendar workout such as `12 x 45 sec @ Threshold` now displays `Today: 12 x 45 sec` with the athlete's Threshold pace target per mile instead of falling back to the Threshold default distance target (`1000m`) or treating `45 sec` as the target time. Coach how-to and regression coverage were updated.
- Follow-up: the timed quality-session prescription parser now uses saved workout details and target splits only, not the coach-entered Training Calendar title. This keeps custom titles such as `12x45 Threshold` from driving target math while still allowing the structured details (`12 x 45 sec @ Threshold`) to produce the correct Threshold pace target. Regression coverage was updated.
- Follow-up: SMARTCoach app target display now rehydrates structured Training Calendar details and target splits from the loaded calendar day when a saved group or runner selection only has thin workout fields. This keeps the athlete Details / Multi Athlete Timer target from falling back to the default Threshold `1000m` target when the selected calendar workout is a timed set such as `12 x 45 sec @ Threshold`. Cached target keys now include the rehydrated workout text/type so stale `1000m` target ranges are invalidated. The coach-entered workout title remains display-only for target math. Regression coverage was updated.
- Follow-up: the `1000m` target was traced to the fallback Threshold/Lactate Threshold pace rule, which is used when the athlete detail target renderer has only `workoutType: Threshold` and cannot see the structured timed set text. The renderer now loads current Training Calendar days with `cache: no-store` before target calculation when a SMART Trak Calendar selection is missing details, then matches the selected day by record/source id, date/title, or title as a last resort so the structured `12 x 45 sec @ Threshold` line reaches target math. Regression coverage was updated.
- SMARTCoach app Refresh no longer reopens an athlete Details screen from the main groups screen. The refresh redraw helper now only refreshes Details when that screen is actually active; otherwise the main screen stays in place after the splash dismisses. Coach how-to and regression coverage were updated.
- Follow-up to the SMARTCoach app Training Calendar refresh fix: Refresh now clears saved group and runner calendar selections when they point to a Training Calendar workout that was removed from the calendar. This handles the case where a workout such as today's deleted `2 x 1 Mile` quality session is no longer visible in SMART Trak but was still saved on the phone app as the selected calendar workout. After cleanup, the app can use the next current Training Calendar workout for the group. Coach how-to and regression coverage were updated.
...
- Attendance Trak now uses the current roster name when returning saved attendance marks that are linked by athlete/contact id. Correcting a misspelled athlete name on the roster updates the displayed Attendance Trak name for linked past marks after refresh, while records without a stable roster id keep their saved name to avoid guessing. Coach how-to and regression coverage were updated.
- Athlete Calendar now treats coach-synced completed workouts as the official completion for matching assigned workouts. The server matches completed SMARTCoach app sync records back to visible Training Calendar assignments by athlete, date, and workout title, and completed syncs outrank older athlete/local skipped or modified statuses. Training Calendar days removed by a coach are also hidden from Athlete Calendar instead of showing as skipped. Coach how-to and regression coverage were updated.
- Equipment Trak numbered inventory now treats **Lost # / qty** as the source of truth for unavailable numbered items. A numbered range marked Lost / Damaged no longer blocks every number in the range from the Issue Sheet or mobile Equip saves; only exact numbers listed in the lost-number field are blocked. Size/count inventory still supports whole-row Lost / Damaged blocking. Coach how-to and regression coverage were updated.
- Training Calendar **Add Easy Run** now leaves **Target** blank by default so the SMARTCoach app uses the standard Easy/Recovery target from each athlete's current fitness. When no selected athlete has current fitness, the saved Easy Run target falls back to `Conversational`; the mobile app also shows `Target: Conversational` for Easy/Recovery Run targets when a specific athlete has no current fitness. Coach how-to and regression coverage were updated.
- SMARTCoach app home-screen/groups-tray **Refresh** now shows the existing full-screen SMARTCoach splash while SMART Trak account data reloads. The small bottom refresh status is cleared at refresh start, the splash is dismissed on success, coach-access-needed, and error paths, and the Settings **Refresh App** full reload remains separate. Coach how-to and regression coverage were updated.
- Training Calendar Quality Session add/edit now hides the top **Effort / Type** control in Quality mode and uses the first work set's **Effort** as the saved workout effort. This removes the duplicate effort choice and prevents the top activity field from disagreeing with the set effort that drives targets. Dashboard Completed Workouts also infers precise quality effort from saved `@ Effort` text or the saved planned effort percent, so a new interval workout with an Aerobic Power storage bucket no longer displays Aerobic Power under the workout title. Coach how-to and regression coverage were updated.
- Dashboard Completed Workouts now treats the saved `Workout:` note label as the coach-editable workout type when a completed record has both a precise quality label and a broad storage category. The SMART Trak object `workout_type` field still stores broad allowed options such as Aerobic Power, but the correction modal now reopens with Interval/Repetition/etc. from the saved note and the detail row hides the broad bucket when it is only the storage category for that precise workout.
- Dashboard Completed Workouts **Correct Training Entry** now exposes **Planned Target** and **Planned Effort** as editable fields. Saving a correction rewrites the saved `Workout:`, `Planned target:`, and `Planned effort:` note lines together, so changing a completed workout from a broad category such as Aerobic Power to Interval no longer reloads with the old target review range. Coach how-to and regression coverage were updated.
- Dashboard Completed Workouts **Correct Training Entry** now includes the missing quality workout options in its Workout Type dropdown: Threshold, Interval, Repetition, Hills, Fast Reps, and Speed Endurance II. This aligns completed-workout corrections with Add/Log Quality Session effort choices. Coach how-to and regression coverage were updated.
- Training Calendar Quality Session edit now uses the same effort dropdown as Add Quality Session. When an edited workout's details contain a set like `3 x 1 mi @ Interval`, the edit modal selects Interval and shows the full Quality effort list instead of only the broader saved category list such as Aerobic Power. Coach how-to and regression coverage were updated.
- SMARTCoach app target calculation now lets explicit Quality Session effort text win over the broader saved workout category. A Training Calendar workout such as `3 x 1 mi @ Interval` saved under the Aerobic Power category now uses the Interval target rule instead of Aerobic Power's 78-86% rule, so future synced 3x1 interval workouts target the interval range. Existing synced rows keep their saved target unless corrected/resynced. Coach how-to and regression coverage were updated, and the mobile app version stamp was bumped.
...
- Current plan breakdown: Essential is $10/month or $100/year; Pro 25 is $19/month or $199/year; Pro 100 is $29/month or $299/year; Pro 200 is $39/month or $399/year; Pro Unlimited is a Custom/manual offer for programs that need unlimited active athletes.
...
483. Added Phase 1 Results Board sharing. Dashboard now has a **Results** share modal with active/off, resettable link, sport/year selection, visible latest-meet/season/PB-SB/team-summary options, board name, and coach message. `/results-board.html` is a read-only public page backed by `/api/smart-trak/results-board`, showing latest meet results plus season-best rows for the selected sport/year with meet, event, gender, and search filters. The board uses only the account's team meet results, not outside team results.
484. Results Board Phase 2 adds a public **Meet Archive** layer. Results Board Sharing now has a Meet Archive visibility toggle, the public API returns meet-level summaries grouped by meet/date, and `/results-board.html` renders a read-only archive table with meet date, result count, athlete count, event count, PB count, and SB count.
485. Results Board Phase 3 adds a public **Athlete Summary** layer. Results Board Sharing now has an Athlete Summary visibility toggle, the public API returns season result totals grouped by athlete, and `/results-board.html` renders a read-only athlete table with result count, meet count, event count, PB count, SB count, best result, and latest result date.
...
497. Meet History season filtering now prefers saved sport/year labels such as **Cross Country 2026** and **Track 2026**, matching the newer Dashboard/Miles/Speed/Results Board season pattern. Older Fall/Spring/year-only labels remain as fallbacks for records without a saved Track or Cross Country sport, and scheduled meet cards now respect their own saved sport/year filters even before result rows exist.
498. Calendar-created race meets now carry the selected sport through the full save flow. Training Calendar sends the selected sport when creating/editing the linked meet, keeps the sport on newly added local calendar days, the Meets API persists sport on both account-backed and custom-object save paths, and the SMARTCoach app reads a calendar day's saved `Sport:` note before falling back to a stale linked meet sport. This prevents a Cross Country race added from Training Calendar from saving as Track in the app and Meet History. Existing meet/result records already saved with the wrong sport still need a one-time correction to Cross Country.
499. Individual meet-result split details are now coach-visible. Dashboard Recent Meet Results opens a read-only detail modal for normal individual race rows, and Meet History adds a **Details** action beside Edit/Void. Both detail views show event, total result, meet/date, best status, notes when present, and saved lap/split times from `splits_json` / `splitsText`; empty split data shows a clear no-splits message. The coach how-to guide now tells coaches to use Details to review saved lap or split times.
500. SMARTCoach app Attendance now has a Phase 1 practice summary. From the Attendance sheet, coaches can tap **Summary** to review the selected group's saved attendance history with group attendance %, Present, Absent, Excused, and Tardy cards plus an athlete dropdown and P/A/E/T table. Checked Out is counted with Present for the summary percentage, matching the existing Attendance Trak math. The coach how-to guide and regression coverage were updated.
501. SMARTCoach app Attendance Summary Phase 2 now loads synced SMART Trak attendance before calculating the selected group summary. The phone app fetches account attendance, filters rows by the selected group's saved id or group name, merges those synced rows with any current phone-saved marks, then calculates each athlete's P/A/E/T totals and group attendance percentage. This fixes the Phase 1 local-cache-only summary that could undercount athletes whose marks were synced from another device or already stored in SMART Trak. The coach how-to and regression coverage were updated.
502. Attendance sync storage now uses itemized registry records instead of rewriting the full account attendance mirror on every save. `saveAttendanceRecords` migrates legacy `attendanceMirror` rows into keyed attendance items, compacts the main account record to a small `lastAttendanceSync` summary, and `loadAttendanceRecords` merges legacy and keyed rows with deleted-row protection. This addresses the mobile “saved on this device, but SMART Trak sync failed: Account registry request failed” path that can happen after attendance history grows large. Account-registry regression coverage verifies legacy migration, compact account saves, and keyed attendance loading.
503. SMARTCoach app Attendance save feedback now appears directly above the **Save Attendance** button instead of in the top attendance count line. Saving and validation messages render in red, successful sync shows a green **Complete** message, and the Attendance sheet closes after a short confirmation delay. The coach how-to and regression coverage were updated.
504. SMARTCoach app Attendance Summary Phase 3 adds an athlete-specific recent marks view. The group summary still uses the merged SMART Trak plus phone-saved attendance rows, and selecting an athlete now shows recent attendance marks by date, checkpoint, status, and note when available so coaches can discuss attendance at practice from the phone. The coach how-to and regression coverage were updated. Per coach direction, this was not added to What's New.
505. SMARTCoach app Attendance Summary now makes the athlete detail path clearer. The summary includes a hint to select an athlete or tap an athlete row, and each summary table row is tappable so coaches can reveal that athlete's recent marks without hunting through the dropdown. The coach how-to and regression coverage were updated. Per coach direction, this was not added to What's New.
506. SMARTCoach app Attendance Summary now has a date range selector. Coaches can review all attendance dates, last 7/14/30 days, or a custom start/end range; the group cards, athlete P/A/E/T table, and selected athlete recent marks all use the same filtered attendance rows. The coach how-to and regression coverage were updated. Per coach direction, this was not added to What's New.
507. SMARTCoach app Attendance Summary selected-athlete recent marks no longer stop after the latest 12 rows. The detail list now renders every mark inside the selected date range, so a custom range starting May 1 can show May attendance once the coach scrolls through the selected athlete's history. Regression coverage prevents the old 12-row cap from returning. Per coach direction, this was not added to What's New.
508. SMARTCoach app Attendance Summary top cards were simplified to three cards on one row: Group attendance, Athletes, and Total days possible. Total days possible counts unique date/checkpoint combinations inside the selected range. Tapping an athlete row still opens that athlete's recent marks, and tapping the selected row again returns to all athletes. The coach how-to and regression coverage were updated. Per coach direction, this was not added to What's New.
509. SMARTCoach app Attendance Summary now includes a Status filter. Coaches can narrow the athlete list and selected-athlete recent marks to Present, Tardy, Checked Out, Excused, or Absent while the top attendance percentage remains based on the selected date range. The coach how-to and regression coverage were updated. Per coach direction, this was not added to What's New.
510. SMARTCoach app Attendance Summary table headers are now sortable. Coaches can tap Name, %, P, A, E, or T to sort ascending/descending after the selected range and status filters are applied, making it easier to find low attendance, high absences, or high tardies during an athlete conversation. The coach how-to and regression coverage were updated. Per coach direction, this was not added to What's New.
511. SMARTCoach app Attendance Summary now de-dupes phone-saved attendance against synced SMART Trak attendance for the same athlete/date/group/checkpoint. SMART Trak rows are preferred when both exist, and duplicate local checkpoints with the same date/name are compacted when opening the summary. This prevents a failed-sync re-entry from appearing twice on the app summary while preserving unsynced phone-only marks. The coach how-to and regression coverage were updated.
512. SMARTCoach app Attendance Summary **Total days possible** now uses the same per-athlete marked-opportunity basis as the table percentages. Instead of counting every unique group date/checkpoint found anywhere in the source rows, it shows the highest marked date/checkpoint count for an athlete in the selected range, preventing partial or stray rows from inflating the card. The coach how-to and regression coverage were updated.
513. Dashboard Log Miles now gives a more specific save confirmation for individual entries, such as `Saved 4 miles for Isabella Wallace.` The message uses the saved payload distance and athlete name, formats miles consistently, and keeps group/quality-session confirmations readable. The coach how-to and regression coverage were updated.
514. Athlete Setup Training Groups save/delete confirmation now appears in the bottom group action row beside the Select All/Clear/Delete/Save Group buttons instead of in the top section header. This keeps feedback near the action the coach just used and prevents the success message from floating away from the controls. The coach how-to and regression coverage were updated.
515. Equipment Trak numbered inventory now blocks only the exact numbers entered in **Lost # / qty**. Marking a numbered inventory row Lost / Damaged no longer makes every number in that range unavailable, while size/count inventory still supports whole-row Lost / Damaged blocking.
515. Dashboard completed-workout rows now recover the workout type from the saved performance-record name when the mapped `workout_type` field is missing after a Log Miles/manual mileage sync fallback. This prevents manual logged miles from showing a blank Workout column while still using the same normalized completed-workout data for Dashboard and Miles Board totals. During tracing, the visible Vanessa mileage gap was also confirmed to be partly date-range based: the Miles Board screenshot was scoped to May 1-Jul 8, while the Dashboard all-activity view included Jul 9 and Jul 10 manual entries.
516. SMARTCoach app Speed Metrics athlete reorder handles no longer force the athlete row open after a drag. The handle path now suppresses the follow-up mobile click that can happen after pointer drag, while preserving the normal tap-athlete behavior for opening or closing rep details. The coach how-to and regression coverage were updated.
517. SMARTCoach app Speed Metrics now includes the drag-handle instruction directly under **Speed Capture**, telling coaches they can drag the handle beside a name to reorder without opening athlete details. Regression coverage checks the visible helper text.
518. SMARTCoach app bottom controls now use a larger shared gesture-safe buffer. Modal sheets, bottom toolbars, and bottom action bars keep the same workflow but sit higher above iPhone/Android swipe-home areas so coaches can swipe up from the bottom without accidentally opening a bottom icon. The coach how-to and regression coverage were updated.
519. SMARTCoach app settings now include **Training/Race Order**. The default **Speed, gender, name** order applies when opening training groups, meet/race groups, and new Speed Metrics sessions: speed/timing signals sort fastest first, then Boys, Girls, Unlisted, then athlete name. Speed Metrics still preserves any manual drag order once the coach reorders that session. Attendance was intentionally left out so its attendance summary filters and sorting remain unchanged. The coach how-to and regression coverage were updated.
520. SMARTCoach app Training/Race Order now re-sorts group timing rows after planned target data finishes loading. The speed-first order can use the same planned target midpoint displayed on the runner row, so athletes with faster target ranges move up instead of falling back to gender/name before async target loading completes. The re-sort is skipped while timers are running and guarded against loops. The coach how-to and regression coverage were updated.
521. SMARTCoach app shared training-group saves now preserve the SMART Trak group roster when the phone app is only using a temporary timing-session roster. The red X on a runner row can remove an athlete from the current timing session without letting a later shared-group save overwrite the official group roster for assistants. App-created groups without an existing SMART Trak group still serialize their runner list, and existing shared groups still filter out inactive athletes before saving. The coach how-to and regression coverage were updated.
522. Dashboard completed-workout corrections now support editing rep and rest split times. The **Correct Training Entry** modal includes a **Reps / Rest Times** field populated from the saved split chips, accepts one split per line, saves the updated split text to the performance record and training mirror, and updates the completed-workout row immediately after save. The coach how-to and regression coverage were updated.
523. SMARTCoach app training/race runner drag now preserves the manual order after drop. The timing screen renders the coach's custom dragged order instead of immediately reapplying speed/gender/name sorting, and later planned-target resorting skips manually ordered sessions. The coach how-to and regression coverage were updated.
...
528. SMARTCoach app target calculation now prioritizes explicit Quality Session effort text such as `3 x 1 mi @ Interval` over the broader saved workout category such as Aerobic Power. This fixes mile interval calendar workouts using Aerobic Power's 78-86% target range instead of the Interval rule. Existing synced rows keep their saved target unless corrected/resynced. The coach how-to and regression coverage were updated, and the mobile app version stamp was bumped.
529. Training Calendar Quality Session edit now repopulates the Effort dropdown from the same Quality effort list used by Add Quality Session and selects the first parsed set effort from saved workout details. This makes edited workouts such as `3 x 1 mi @ Interval` show Interval, Repetition, Hills, Fast Reps, and the other Quality efforts instead of only the broader saved category options. The coach how-to and regression coverage were updated.
530. Dashboard Completed Workouts **Correct Training Entry** now includes the missing quality workout options in its Workout Type dropdown: Threshold, Interval, Repetition, Hills, Fast Reps, and Speed Endurance II. This aligns completed-workout corrections with Add/Log Quality Session effort choices. The coach how-to and regression coverage were updated.
531. Athlete Calendar now treats coach-synced completed workouts as the official completion for matching assigned workouts. The server matches completed SMARTCoach app sync records back to visible Training Calendar assignments by athlete, date, and workout title, and completed syncs outrank older athlete/local skipped or modified statuses. Training Calendar days removed by a coach are also hidden from Athlete Calendar instead of showing as skipped. The coach how-to and regression coverage were updated.
532. Attendance Trak now resolves saved attendance names from the current active roster when the attendance mark has a saved athlete/contact id. This means a roster spelling correction updates linked past attendance rows on refresh. Marks without a stable roster id keep their saved name instead of being guessed from spelling. The coach how-to and regression coverage were updated.
533. SMARTCoach subscription pricing was updated: Essential remains $10/month or $100/year; Pro 25 is now $19/month or $199/year; Pro 100 is now $29/month or $299/year; Pro 200 is now $39/month or $399/year; Pro Unlimited remains Custom. Plan definitions, onboarding amount defaults, backend standard-price handling, the public sales page, the coach how-to guide, and regression coverage were updated.
534. SMARTCoach app calendar-workout refresh now advances selected Training Calendar groups to the current upcoming calendar workout for that group. A group marked `calendar-selected` no longer blocks app refresh from replacing an old selected calendar day, and stale runner-level calendar overrides are cleared so athlete detail targets match the current SMART Trak Training Calendar. The coach how-to and regression coverage were updated.
535. Follow-up: SMARTCoach app refresh now also rebinds and redraws any currently open athlete detail/profile screen after SMART Trak data reloads, so an open profile does not keep showing a stale selected calendar workout target. Regression coverage was updated.
536. SMARTCoach Admin Strava Test now waits for the Dashboard roster before rendering the **Import To Athlete** dropdown after loading recent Strava activities. If the Strava modal opens before Dashboard data finishes loading, **Load Recent Activities** now briefly shows `Loading dashboard roster for import...`, calls the existing dashboard loader, and only then renders athlete choices. This prevents the dropdown from getting stuck on `Load the dashboard roster first`. This is owner/admin-only, so the coach how-to guide was not changed.
537. SMARTCoach Admin Strava Test import now refreshes the Dashboard roster every time before rendering the **Import To Athlete** dropdown, so athletes added before opening the modal appear in the selector instead of relying on stale in-memory rows. Strava imports now default Surface to `Road` instead of `Unspecified`, and the sync-session save path can retry without Surface if GHL rejects the option. This fixes the Strava import error `'unspecified' isn't an allowed option for Surface.` This is owner/admin-only, so the coach how-to guide was not changed.
538. SMARTCoach Admin Strava imports now have a stronger duplicate guard. The sync-session duplicate check still searches SMART Trak performance records by `source_record_id`, and now also checks the account training mirror for the same source record before saving. The Strava Test import button also disables during the import request to prevent double-click/race duplicates. Existing intentionally duplicated imports remain visible and can be voided manually; new repeats of the same Strava activity for the same athlete should return the duplicate-sync message instead of creating another completed workout.
539. SMARTCoach Admin Strava Test now marks imported activities before the owner/admin clicks import again. The activity list preserves the selected SMART Trak athlete, compares each Strava activity id against loaded completed-workout `sourceSessionId`/`sourceRecordId` rows for that athlete, shows **Already Imported**, and disables the import button for matching rows. After a successful import, the Dashboard reloads and the Strava activity list rerenders so the imported row immediately changes to the already-imported state. This is owner/admin-only, so the coach how-to guide was not changed.
540. SMARTCoach Admin Strava import review now recognizes the saved backend slug form for Strava session ids. The modal checks both `strava_<activity id>` and `strava-<activity id>` against completed workout rows, so previously imported Strava activities should show **Already Imported** for the selected athlete instead of a disabled-looking import button. The admin shortcut was also moved from the Dashboard action row into the Training Load area and shortened from **Strava Test** to **Strava**. This is owner/admin-only, so the coach how-to guide was not changed.
541. SMARTCoach beta admin tools are now limited to the `tca-trackandcc` beta account by default. The Dashboard **API Usage** button and Training Load **Strava** button stay hidden for other accounts, and the matching API Usage/Strava admin endpoints also require the beta account gate after owner/admin access is confirmed. The backend allowlist can be expanded with `SMARTCOACH_BETA_ACCOUNT_KEYS` if another beta account is added later. This is owner/admin-only, so the coach how-to guide was not changed.
...
546. Training Calendar now owns the beta owner/admin **Strava** import modal instead of bouncing to Dashboard. The Training page Strava button opens the local modal, OAuth can safely return to `/training-calendar.html#strava`, recent activities refresh SMART Trak roster/completed-workout data from the Dashboard API before showing the athlete picker, and imports still use the existing sync-session payload and duplicate detection. The Dashboard Strava modal remains as a fallback, but the normal beta entry point no longer forces a Dashboard reload. This is owner/admin-only, so the coach how-to guide was not changed.
547. Training Calendar Strava beta imports now look for a same-day Training Calendar workout for the selected athlete before saving. The Strava modal compares the activity date against athlete-specific and group calendar days, shows whether the activity matched a plan, and sends the matched group, sport, workout type, phase, plan/day ids, calendar workout title, and planned volume through the existing sync-session payload. Unmatched Strava activities still import as a Strava Easy/Recovery Run. This is owner/admin-only beta work, so the coach how-to guide was not changed.
548. Dashboard Log Miles duplicate checking now sends a manual-mileage source session id built from the saved date, group, workout type, log type, distance, time, source, and quality-session layout before the shared sync duplicate guard runs. This prevents a different same-day manual entry from being blocked by an earlier manual save while preserving the already-synced warning for exact repeat submissions. The Log Miles coach guide and regression coverage were updated.
549. Dashboard weekly and filtered athlete volume now count saved completed-volume totals that are stored as unitless completed values such as `3.24 completed`. Those rows were already visible in Completed Workouts, but weekly/filtered volume could add zero unless the saved value included an explicit unit such as `mi`. Backend normalization and Dashboard fallback parsing now treat those unitless completed totals as miles, and the coach how-to guide plus regression coverage were updated.
550. Dashboard Completed Workouts **Correct Training Entry** and Log Miles Easy Miles now make coaches choose the completed-volume unit. Both controls start on **Select unit** and block save until miles, kilometers, or meters is chosen, so rows save as values like `1.01 mi` instead of ambiguous unitless values like `1.01`. Log Miles field-info bubbles now use the fixed-position tooltip helper instead of an off-screen CSS bubble. The modal controls, local save payload, coach how-to guide, and regression coverage were updated. What's New was not updated.
551. Existing completed-workout rows with unitless completed volume now display as miles in Dashboard volume cells and open **Correct Training Entry** with Miles selected. This lets coaches correct old rows saved as `4` and save them back as `4 mi`, while brand-new blank entries still require an explicit unit selection before saving. The dashboard API display label, browser fallback display label, correction modal preload, coach how-to guide, and regression coverage were updated. What's New was not updated.
552. Follow-up to Miles Trak API dedupe: preserve all live SMART Trak performance records by object id, and only remove training-mirror backup rows whose `source_record_id` is already present in live records. The prior broad source-id dedupe was too aggressive and dropped legitimate live rows with shared older source ids, which moved Hayden from 231/too high to 208.6/too low. Regression coverage now protects the live-plus-mirror merge behavior.
553. Dashboard completed-volume parsing now handles leading-decimal mileage values. Saved values such as `.19 mi` count as 0.19 miles instead of 19 miles, unitless `.78` fallback totals count as 0.78 miles, and the unit parser no longer grabs numbers that are part of another decimal or a time-like value. The Dashboard API, browser fallback display/reload path, correction-modal preload, coach how-to guide, and regression coverage were updated. What's New was not updated.
554. Dashboard correction notes cleanup now removes generated training-review lines from the editable Notes field and from rebuilt correction notes, including Planned volume, Actual, Difference, Current fitness, Workout Location, correction audit labels, and old literal escaped-newline variants. This prevents repeated target-review text from appearing in the modal or being re-saved as coach notes. Regression coverage was updated. Coach how-to and What's New were not updated.
555. SMARTCoach app Equipment Trak issue list now filters out inactive roster athletes and lets coaches choose the issue-day order: Name A-Z, Girls/Boys/Name, or Grade/Name. The order preference is stored per account on the device, and sorted cards save back to the correct athlete records. A follow-up bumped the mobile app version/build stamp so installed phone shells pick up the new Equipment screen. The coach how-to guide and regression coverage were updated. What's New was not updated per user instruction.
556. SMARTCoach app Equipment Trak now labels the in-modal reload control **Refresh Data**. That button reloads Equipment Trak records for the selected group; it does not reload the installed app shell. The coach how-to guide and regression coverage were updated. What's New was not updated per user instruction.
557. Dashboard correction notes cleanup now treats Strava import/audit lines as system metadata. Strava activity ids, matched workout/group lines, activity summary lines, Strava lap diagnostics, and Strava import status messages no longer appear in the editable Notes field or duplicate when corrections are saved, while `Rep pace details:` continues to populate **Actual Rep Paces**. Regression coverage was updated. Coach how-to and What's New were not updated because this is owner/admin Strava beta cleanup.
558. SMART Trak athlete roster normalization now excludes contacts tagged `smartcoach-prospect` in the Athletes API, Dashboard roster/count builder, and Athlete Calendar matching. This lets the main SMARTCoach Pro/admin sub-account hold coach-prospect campaign contacts and Coach Info fields without those prospects appearing as athletes in roster views. Regression coverage was updated. Coach how-to and What's New were not updated because this is internal campaign/contact hygiene.
559. Dashboard Completed Workouts Target Review now uses an athlete's current fitness as a display-only fallback for completed Easy/Recovery Run and Long Run mileage rows without a saved planned target. If the row has completed distance and time, SMART Trak compares actual pace against the expected current-fitness pace, including manual mileage and synced/non-calendar entries. Saved targets, saved differences, split/rep reviews, and Actual Rep Paces still win, and the generated fallback is not saved back into correction notes. The coach how-to guide and regression coverage were updated. What's New was not updated per user instruction.
560. Dashboard season filtering now separates Cross Country from Track/Summer season buckets. Cross Country rows, including older rows whose group or workout text clearly says CC/XC/Cross Country, show under `Cross Country 2026`; Track rows stay in the date bucket, so July Track rows show under `Summer 2026`. `Current season` now matches only the active current-season keys instead of every row from the current year, so Spring 2026 does not get pulled into the current view just because it shares the year. The coach how-to guide and regression coverage were updated. What's New is pending user approval.
561. Follow-up to Dashboard season filtering: summer/fall non-track training rows now resolve to the cross country season year even when older saved rows do not explicitly say CC/XC/Cross Country. This keeps summer cross country mileage/workouts under `Cross Country 2026` instead of creating a `Summer 2026` cross country bucket, while explicit Track rows still remain in the normal date bucket such as `Summer 2026`. Dashboard season choices, current-season filtering, and reload/render all use the same row season key. The coach how-to guide and regression coverage were updated. What's New is pending user approval.
562. Dashboard Cross Country season filtering was tightened again after the Aug 1 dashboard showed only 77 workouts under `Cross Country 2026`: non-track training rows from May through November now resolve to the cross country season year, so May cross country mileage is not left behind in `Spring 2026`. Track rows remain in calendar seasons (`Winter 2026`, `Spring 2026`, `Summer 2026`) based on date. The coach how-to guide and regression coverage were updated. What's New is pending user approval.
563. Dashboard season filtering was corrected after the Aug 1 follow-up overclassified summer manual/track mileage as Cross Country. The Dashboard no longer infers Cross Country from month or from the athlete's current roster/season identity; completed workout rows use the sport/season saved on the row, so `Cross Country 2026`, `Track 2027`, and `Cross Country 2027` remain separate historical buckets as athletes move between seasons. Log Miles now requires the coach to choose Cross Country or Track before saving; Cross Country entries save as `Cross Country <year>`, while Track entries save with the track calendar season for the workout date. Manual-mileage API fallback no longer silently defaults missing sport to Cross Country. The coach how-to guide and regression coverage were updated. What's New is pending user approval.
564. SMARTCoach app Attendance now filters the displayed, summarized, mark-all, and synced attendance runner list through the active roster. Inactive roster members such as Mark Sample are not shown in Attendance after the app refreshes roster data, even if their old local group row remains on the device. The active-roster matcher now requires `smartcoachActive === true` instead of treating every returned roster member as active. The mobile app version stamp, coach how-to guide, and regression coverage were updated. What's New is pending user approval.
565. Attendance inactive-runner follow-up: SMARTCoach now recognizes broader inactive roster values such as `Inactive Runner`, `Archived`, `Former`, `Graduated`, and `Quit` when normalizing athletes. The phone app Attendance flow waits for the active roster before showing or saving attendance, and it blocks workout auto-attendance for inactive runners. SMART Trak Attendance already filters saved rows through the active roster, so older inactive-athlete marks no longer appear in the normal report once roster normalization identifies the athlete as inactive. The mobile app version stamp, coach how-to guide, and regression coverage were updated. What's New is pending user approval.
566. Training Calendar Strava beta activity loading now requests up to 30 recent activities from the last 45 days for the selected athlete and shows a specific empty-state message when Strava returns no recent activities for a connected athlete. This clarifies Garmin Connect vs Strava differences: SMARTCoach can only import what Strava exposes for the connected Strava account. The athlete-activities backend also accepts optional `after`/`before` query bounds. Regression coverage was updated. The coach how-to guide and What's New were not updated because this remains private beta/admin-only Strava tooling.
567. Dashboard now includes a temporary owner/admin-only Current Fitness Cleanup tool behind admin tools. The tool compares each active athlete's current fitness mark with the latest race, projects the latest-race time from the current mark, shows the previous race result when available, and lets the owner/admin either restore the previous race or use the latest race through the existing Athlete Best save path. Dashboard current-fitness payload now includes the Athlete Best record id so cleanup actions replace the exact saved current-fitness row instead of creating duplicates. Regression coverage was updated. The coach how-to guide and What's New were not updated because this is temporary owner/admin cleanup.
568. Dashboard Current Fitness Cleanup follow-up: the temporary Fitness Review modal now uses a dedicated wide/tall panel instead of the narrow What's New panel, and its review table has its own horizontal workspace. This makes the current mark, latest race, projection, previous race, review, and actions easier to see while cleaning up current-fitness marks. Regression coverage was updated. The coach how-to guide and What's New were not updated because this remains temporary owner/admin cleanup.
569. Dashboard current-fitness projections now parse the visible current-fitness result display before falling back to hidden numeric milliseconds. This prevents stale `resultMs` data from producing a projection that does not match the mark shown to the coach, such as a displayed 5:09 mile projecting like a slower mile. The event-distance helper also recognizes plain `Mile`, `1600`, and `3200` aliases. Regression coverage was updated. The coach how-to guide and What's New were not updated because this is calculation correctness for the temporary review/target helpers.
570. Dashboard Current Fitness Cleanup now projects from the previous race when the latest race already matches the saved current-fitness mark. This avoids circular cleanup rows where a latest 2 Mile current mark projected against itself and appeared "usable" even though the previous 1 Mile mark would project a faster 2 Mile. Regression coverage was updated. The coach how-to guide and What's New were not updated because this remains temporary owner/admin cleanup.
571. Dashboard Current Fitness Cleanup follow-up: the latest-race overwrite matcher now compares parsed dates and allows a small result-time tolerance instead of requiring exact display text. This catches rows where current fitness saved as `11:39.0` and the latest meet row displays `11:38.3`, so cleanup can project from the previous mark instead of circularly projecting from the latest race. Regression coverage was updated. The coach how-to guide and What's New were not updated because this remains temporary owner/admin cleanup.
572. Training Calendar Strava beta ordinary-run imports now save completed volume and total time from the same Strava standard-mile split rows that SMARTCoach displays when those split rows are available. This keeps an easy run row from showing split chips that match Garmin Connect while the saved row total still uses a slightly different Strava activity-summary distance/time. Matched rep/rest workouts still use the matched SMART Trak workout layout first. Regression coverage was updated. The coach how-to guide and What's New were not updated because this remains private beta/admin-only Strava tooling.
