...
- SMARTCoach app Equipment Trak now shows only active roster athletes in the phone issue list once roster data is loaded, and adds **Athlete Order** with Name A-Z, Girls/Boys/Name, and Grade/Name options. The selected order is saved per account on the device. Sorted issue cards still save back to the correct athlete records by original runner index. Follow-up bumped the mobile app version/build stamp after the phone still showed the old Equipment screen. Coach how-to and regression coverage were updated. What's New was not updated per user instruction.
