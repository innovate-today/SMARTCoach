...
356. The shared desktop feedback widget now hides external LeadConnector/GHL chat widgets in embedded SMART Trak pages instead of lifting them above the app, and it attempts to hide common GHL parent chrome icons when browser same-origin rules allow access. The LeadConnector chat loader is skipped in embedded mode so coaches see SMART Trak's own Feedback button without extra GHL phone/notification/help icons. Regression coverage checks the embedded hide guard.
357. Added `how-to.html` as an in-app SMART Trak How To Guide page. It fetches and renders `SMART_TRAK_COACH_HOW_TO.md` directly so the website guide stays current while the markdown file remains available for the chat prompt. The Dashboard now has a **How To** button wired through `smartCoachPageUrl`, and regression coverage checks the page, markdown fetch, text-guide fallback, and Dashboard link.
