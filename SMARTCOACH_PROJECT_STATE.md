...
358. SMARTCoach mobile app Account settings now includes **Switch Account**. It clears the remembered account key, account URL override, access code/session, and selected coach identity for the current account on that device, then reloads the app without deleting account-scoped groups/archive data. This gives coaches a clean way out after testing another account on the same phone/browser.
359. SMARTCoach mobile app Account settings now includes **Recover Phone Groups** for the transition to account-scoped phone storage. It lets a coach explicitly merge older phone-only groups from the previous shared local cache into the currently selected account without silently leaking those groups into every account and without deleting existing account-scoped groups.
