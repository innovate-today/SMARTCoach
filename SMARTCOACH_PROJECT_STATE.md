...
355. **Email Calendar Links** now includes an **Email Provider** selector with Default mail app, Gmail, Outlook, and Yahoo Mail. Default still uses `mailto:` for coaches with a local/default mail app configured; Gmail/Outlook/Yahoo use web compose URLs so coaches working inside the GHL embed can open a real provider compose window instead of relying on Chrome's mailto handler. Tests cover the provider URLs and keep `mailto:` from forcing a blank tab.
