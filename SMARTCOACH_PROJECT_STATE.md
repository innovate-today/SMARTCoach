...
- Open decision: Speed Metrics is now saved to Speed Trak, but dashboard training-load/volume counting still needs a coach rule. Decide whether speed volume should be only timed sprint distance (for example, `5 x 30m = 150m`) or a coach-entered session volume that can include warmup/cooldown, and whether one saved Speed Metrics session counts as a completed workout for every athlete with at least one saved rep.
