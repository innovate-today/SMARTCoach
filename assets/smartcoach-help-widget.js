(function(){
  ...
  var css = [
    ...
    '.smartcoach-help-link{border:1px solid #dbe3ef;border-radius:8px;background:#f8fafc;color:#173891;font:900 14px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;padding:10px 11px;text-decoration:none;display:flex;align-items:center;justify-content:center}' +
    '.smartcoach-help-link:hover{background:#eef4ff}' +
  ...
  function createChatLauncher(){
    if(document.getElementById('smartcoachLiveChatBtn'))return;
    var button=document.createElement('button');
    button.id='smartcoachLiveChatBtn';
    button.className='smartcoach-livechat-btn';
    button.type='button';
    button.textContent='Help Chat';
    button.addEventListener('click',function(){
      button.disabled=true;
      button.textContent='Opening Chat';
      loadChatWidget();
      setTimeout(function(){button.hidden=true;},1800);
    });
  ...
  function smartCoachPageUrl(path){
    try{
      ...
      var account=accountKey();
      if(account&&account!=='default')url.searchParams.set('account',account);
      var params=new URLSearchParams(window.location.search||'');
      if(params.get('embed')==='1'||params.get('iframe')==='1'||params.get('embedded')==='1')url.searchParams.set('embed','1');
  ...
  function createBugTrak(){
    ...
    var howTo=overlay.querySelector('#smartcoachHowToLink');
