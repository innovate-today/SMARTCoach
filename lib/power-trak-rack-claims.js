...
function validateRackSessionTransitions(options = {}) {
  ...
  (options.incomingRackSessions || []).forEach((incoming) => {
    ...
    const existingRevision = existing ? Math.max(0, Number.parseInt(existing.revision, 10) || 0) : 0;
    const incomingRevision = Math.max(0, Number.parseInt(incoming.revision, 10) || 0);
...
function advanceRackSessionRevisions(existingRackSessions, incomingRackSessions) {
  const existingById = new Map((existingRackSessions || []).filter(Boolean).map((rack) => [rack.id, rack]));
  return (incomingRackSessions || []).filter(Boolean).map((rack) => {
    const existing = existingById.get(rack.id);
    const revision = Math.max(0, Number.parseInt(existing && existing.revision, 10) || 0) + 1;
    return { ...rack, revision };
  });
}

module.exports = { activeRackForAthlete, advanceRackSessionRevisions, normalizeRackReservations, normalizeRackTombstones, updateRackAthleteReservation, validateRackSessionClaims, validateRackSessionTransitions };
