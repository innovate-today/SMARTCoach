...
function validateRackSessionTransitions(options = {}) {
  ...
  (options.incomingRackSessions || []).forEach((incoming) => {
    ...
    if (deletedIds.has(incoming.id)) {
      throw rackClaimError(`${incoming.rackName || "This rack"} was deleted and cannot be restored by an older rack update.`);
    }
    ...
    if (existing && Number.isFinite(existingUpdatedAt) && Number.isFinite(incomingUpdatedAt) && incomingUpdatedAt < existingUpdatedAt) {
      throw rackClaimError(`${existing.rackName || "This rack"} has a newer saved update. Refresh before saving again.`);
    }
    if (existing && existing.status === "complete" && incoming.status === "active") {
      throw rackClaimError(`${existing.rackName || "This rack"} has already been completed and cannot be reopened by an older rack update.`);
    }
  });
}
