...
async function athleteCalendar(req, res, context) {
  ...
  const visible = mergeVisibleDays(days
    .filter((day) => athleteCanSeeDay(athlete, groups, day) && trainingDayHasActiveMeet(day, activeMeetIds))
...
async function loadSubmittedTrainingStatuses(accountKey, athlete) {
  ...
  return (Array.isArray(mirror) ? mirror : []).map((item) => {
    ...
    if (!note || note.toLowerCase().indexOf("athlete calendar submission") < 0) return null;
...
function submittedStatusFromNote(note) {
  ...
  if (/status:\s*skipped/i.test(note)) return "Skipped";
  if (/status:\s*modified/i.test(note)) return "Modified / correction";
  if (/status:\s*completed/i.test(note)) return "Completed";
  return text.indexOf("athlete calendar submission") >= 0 ? "Completed" : "";
}
