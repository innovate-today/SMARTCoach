...
const { getGhlContext, coachCodeAllowed, coachSessionFromRequest, subscriptionAccessAllowed, subscriptionBlockedMessage, accountAccessAllowed, accountAccessBlockedMessage } = require("./ghl-account");
const { attachRegistryAccount, setSmartTrakSecurityHeaders } = require("./smart-trak-request");
const { loadAccountRecord, loadAccountScopedRecord, loadTrainingMirror } = require("./account-registry");
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!accountAccessAllowed(context.accountAccess)) {
      res.status(403).json({ error: accountAccessBlockedMessage(context.accountAccess), accountAccessRequired: true, accountAccessStatus: context.accountAccess && context.accountAccess.status });
      return;
    }
...
async function athleteCalendar(req, res, context) {
  ...
  const submittedStatuses = await loadSubmittedTrainingStatuses(context.accountKey, athlete);
  const questionSettings = await loadAthleteCalendarQuestionSettings(context.accountKey);
  const visible = mergeVisibleDays(days
    .filter((day) => !isRemovedTrainingDay(day) && athleteCanSeeDay(athlete, groups, day) && trainingDayHasActiveMeet(day, activeMeetIds))
    .map((day) => applySubmittedTrainingStatus(day, submittedStatuses))).map(publicDay);
  res.status(200).json({
    success: true,
    athlete: publicAthlete(athlete),
    days: visible,
...
async function loadSubmittedTrainingStatuses(accountKey, athlete) {
  if (!accountKey || accountKey === "default") return [];
  const mirror = await loadTrainingMirror(accountKey);
  const athleteId = clean(athlete && athlete.id).toLowerCase();
  const athleteName = clean(athlete && athlete.name).toLowerCase();
  return (Array.isArray(mirror) ? mirror : []).map((item) => {
    const props = item && item.properties || {};
    const note = clean(props.coach_note);
    const contactId = clean(props.athlete_contact).toLowerCase();
    const name = clean(props.athlete_name_snapshot).toLowerCase();
    if (!note) return null;
    if (note.toLowerCase().indexOf("smartcoach status: voided") >= 0) return null;
    if (athleteId && contactId && contactId !== athleteId) return null;
    if (!contactId && athleteName && name && name !== athleteName) return null;
    const status = submittedStatusFromNote(note);
    if (!status) return null;
    return {
      date: clean(props.session_date),
      title: submittedWorkoutTitleFromNote(note),
      status,
      updatedAt: clean(item && (item.updatedAt || item.createdAt)),
    };
  }).filter(Boolean);
}

function applySubmittedTrainingStatus(day, submittedStatuses) {
  const match = findSubmittedTrainingStatus(day, submittedStatuses);
  return match && statusPriority(match.status) > statusPriority(day && day.status) ? { ...day, status: match.status } : day;
}

function findSubmittedTrainingStatus(day, submittedStatuses) {
  const dayDate = clean(day && day.date).slice(0, 10);
  const dayTitle = clean(day && day.title).toLowerCase();
  return (Array.isArray(submittedStatuses) ? submittedStatuses : [])
    .filter((item) => clean(item.date).slice(0, 10) === dayDate)
    .filter((item) => !item.title || !dayTitle || clean(item.title).toLowerCase() === dayTitle)
    .sort((a, b) => statusPriority(b.status) - statusPriority(a.status) || clean(b.updatedAt).localeCompare(clean(a.updatedAt)))[0] || null;
}

function submittedStatusFromNote(note) {
  const text = clean(note).toLowerCase();
  const athleteSubmission = text.indexOf("athlete calendar submission") >= 0;
  if (athleteSubmission && /status:\s*skipped/i.test(note)) return "Skipped";
  if (athleteSubmission && /status:\s*modified/i.test(note)) return "Modified / correction";
  if (/status:\s*completed/i.test(note)) return "Completed";
  return submittedWorkoutTitleFromNote(note) ? "Completed" : "";
}

function submittedWorkoutTitleFromNote(note) {
  const lines = clean(note).split(/\r?\n/).map(clean).filter(Boolean);
  const workoutLine = lines.find((line) => /^workout:\s*/i.test(line));
  return workoutLine ? clean(workoutLine.replace(/^workout:\s*/i, "")) : "";
}
...
function buildSyncPayload({ athlete, day, status, actualVolume, totalTime, notes }) {
  const completedVolume = status === "skipped" ? "0 mi" : raceCompletedVolume(day, actualVolume) || actualVolume || day.plannedVolume || "Completed as planned";
...
function raceCompletedVolume(day, actualVolume) {
  ...
  const entered = clean(actualVolume);
  if (entered && /\btotal\b/i.test(entered)) return entered;
  const raceMiles = parseRaceDistanceMiles([entered, day && day.plannedVolume, day && day.targetSplits, day && day.title, day && day.details].filter(Boolean).join("\n"));
  const supportMiles = raceSupportMiles([day && day.details, day && day.targetSplits, day && day.coachNotes].filter(Boolean).join("\n"));
  const totalMiles = raceMiles + supportMiles;
...
function isRaceDay(day) {
  ...
  return /\b(race|meet|time trial)\b/.test(text);
}
...
function raceSupportMiles(text) {
  ...
  const patterns = [
    /\b(?:warmup|warm-up|cooldown|cool-down)\s*:?\s*([\d.]+)\s*(mi|mile|miles|km|kilometer|kilometers|m|meter|meters)\b/gi,
    /\b([\d.]+)\s*(mi|mile|miles|km|kilometer|kilometers|m|meter|meters)\s*(?:warmup|warm-up|cooldown|cool-down)\b/gi,
  ];
  patterns.forEach((pattern) => {
    let match;
    while ((match = pattern.exec(source))) {
      total += distanceToMiles(match[1], match[2]);
    }
  });
  return total;
}
...
function parseRaceDistanceMiles(text) {
  ...
  source = source.replace(/\b(?:warmup|warm-up|cooldown|cool-down)\s*:?\s*[\d.]+\s*(?:mi|mile|miles|km|kilometer|kilometers|m|meter|meters)\b/g, " ");
  source = source.replace(/\b[\d.]+\s*(?:mi|mile|miles|km|kilometer|kilometers|m|meter|meters)\s*(?:warmup|warm-up|cooldown|cool-down)\b/g, " ");
  if (/\bhalf\s*marathon\b/.test(source)) return 13.1094;
  if (/\bmarathon\b/.test(source)) return 26.2188;
  const km = source.match(/\b(\d+(?:\.\d+)?)\s*k\b/);
  if (km) return distanceToMiles(km[1], "km");
  const miles = source.match(/\b(\d+(?:\.\d+)?)\s*(?:mi|mile|miles)\b/);
  if (miles) return distanceToMiles(miles[1], "mi");
  const meters = source.match(/\b(\d+(?:\.\d+)?)\s*(?:m|meter|meters)\b/);
  if (meters) return distanceToMiles(meters[1], "m");
  return 0;
}

function distanceToMiles(value, unit) {
  const number = Number(value);
  const label = clean(unit).toLowerCase();
  if (!Number.isFinite(number) || number <= 0) return 0;
  if (label.indexOf("km") === 0 || label.indexOf("kilometer") === 0) return number * 0.621371;
  if (label === "m" || label.indexOf("meter") === 0) return number / 1609.344;
  return number;
}

function formatMiles(value) {
  const rounded = Math.round(value * 100) / 100;
  if (Math.abs(rounded - Math.round(rounded)) < 0.01) return `${Math.round(rounded)} mi`;
  return `${rounded.toFixed(rounded >= 10 ? 1 : 2).replace(/0$/, "").replace(/\.$/, "")} mi`;
}
...
async function loadGroups(context) {
  try {
    if (context.accountKey && context.accountKey !== "default") {
      const scoped = await loadAccountScopedRecord(context.accountKey, "groups");
      const savedScoped = scoped && scoped.record;
      if (savedScoped && typeof savedScoped === "object") {
        return normalizeGroups(savedScoped.groups);
      }
...
function statusPriority(status) {
  ...
  if (text.indexOf("completed") >= 0) return 5;
  if (text.indexOf("modified") >= 0 || text.indexOf("correction") >= 0) return 4;
  if (text.indexOf("skipped") >= 0) return 3;
  if (text.indexOf("scheduled") >= 0) return 1;
  return 0;
}

function isRemovedTrainingDay(day) {
  const status = clean(day && day.status).toLowerCase();
  const notes = clean(day && day.coachNotes).toLowerCase();
  return status.indexOf("skipped") >= 0 && notes.indexOf("removed from calendar") >= 0;
}
...
function contactName(contact) {
  return displayNameCase(clean(contact && (contact.name || contact.contactName || contact.fullName)) || [contact && contact.firstName, contact && contact.lastName].map(clean).filter(Boolean).join(" "));
}
...
function normalizeGroups(groups) {
  return (Array.isArray(groups) ? groups : []).map((group) => ({
    ...
    athletes: Array.isArray(group && group.athletes) ? group.athletes.map((item) => ({ contactId: clean(item && item.contactId), name: displayNameCase(item && item.name) })) : [],
...
function inactiveValue(value) {
  ...
  if (["no", "false", "0", "inactive", "off"].includes(text)) return true;
