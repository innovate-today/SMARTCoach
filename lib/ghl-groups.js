...
const { getGhlContext, requireProPlan, coachSessionFromRequest } = require("./ghl-account");
const { attachRegistryAccount, setSmartTrakSecurityHeaders } = require("./smart-trak-request");
const { loadAccountRecord, loadAccountScopedRecord, saveAccountScopedRecord, loadCoachDeviceSession } = require("./account-registry");
...
module.exports = async function handler(req, res) {
  ...
  if (rackScoped && req.method !== "GET") {
    res.status(403).json({ error: "Rack devices cannot change training groups." });
    return;
  }
  if (!token || !locationId) {
  ...
  try {
    if (req.method === "GET") {
      ...
      const groups = rackScoped ? state.groups.map(publicRackGroup) : state.groups;
      res.status(200).json({ success: true, groups, updatedAt: state.updatedAt });
      return;
    }
...
function publicRackGroup(group) {
  return {
    id: clean(group.id),
    name: clean(group.name),
    athletes: (Array.isArray(group.athletes) ? group.athletes : []).map((athlete) => ({
      id: clean(athlete.id || athlete.smartcoachAthleteId || athlete.contactId),
      name: clean(athlete.name),
    })),
  };
}
...
async function loadGroupsState({ token, locationId, accountKey }) {
  if (accountKey && accountKey !== "default") {
    const scoped = await loadAccountScopedRecord(accountKey, "groups");
    const savedScoped = scoped && scoped.record;
    if (savedScoped && typeof savedScoped === "object") {
      return {
        groups: normalizeGroups(savedScoped.groups),
        updatedAt: clean(savedScoped.updatedAt),
      };
    }
...
async function saveGroupsState({ token, locationId, accountKey, groups }) {
  if (accountKey && accountKey !== "default") {
    ...
    await saveAccountScopedRecord(accountKey, "groups", { groups, updatedAt });
    return { groups, updatedAt };
  }
...
function mergeGroups(existingGroups, incomingGroups, deleteGroupIds) {
  ...
  normalizeGroups(existingGroups).forEach((group) => {
    ...
    if (key && !deleted[group.id]) {
      ...
      const rosterKey = groupRosterKey(group);
      if (nameKey) nameToKey.set(nameKey, key);
      if (rosterKey) rosterToKey.set(rosterKey, key);
    }
  });
  normalizeGroups(incomingGroups).forEach((group) => {
    ...
    const key = previousNameKey || previousRosterKey || groupMergeKey(group);
    if (!key) return;
...
function groupRosterKey(group) {
  const athletes = normalizeAthletes(group && group.athletes);
  const athleteKeys = athletes.map((athlete) => clean(athlete.contactId || athlete.smartcoachAthleteId || athlete.name).toLowerCase()).filter(Boolean).sort();
  if (!athleteKeys.length) return "";
  const type = clean(group && group.type) === "meet" ? "meet" : "training";
  const season = clean(group && group.season).toLowerCase();
  const seasonYear = Number(group && group.seasonYear) || "";
  return ["roster", type, season, seasonYear, athleteKeys.join("|")].join(":");
}
