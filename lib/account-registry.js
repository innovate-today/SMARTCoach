...
async function saveAttendanceRecords(accountKey, records) {
  const items = Array.isArray(records) ? records.map(normalizeAttendanceRecord).filter(Boolean) : [];
  if (!items.length) return { saved: false, count: 0, reason: "No attendance records to save." };
...
function normalizeAttendanceStatus(status) {
  ...
  if (["present", "late", "excused", "absent"].includes(value)) return value;
  return "";
}
...
function coachDeviceUsageSummary(devices) {
  ...
  const list = (Array.isArray(devices) ? devices : []).filter((item) => item && item.deviceId).sort((a, b) => clean(b.lastSeenAt).localeCompare(clean(a.lastSeenAt)));
  ...
  return {
    ...
    activeDevices: list.filter((item) => withinDays(item, 30)).length,
    devicesSeenThisWeek: list.filter((item) => withinDays(item, 7)).length,
