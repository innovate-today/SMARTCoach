...
async function mirrorSchoolRecords(accountKey, records, options = {}) {
  ...
  for (const item of items) {
    ...
    await registryRequest(config, ["sadd", indexKey, key]);
  }
  for (const id of deleteIds) {
    await registryRequest(config, ["del", recordsMirrorItemKey(config, accountKey, id)]);
    await registryRequest(config, ["srem", indexKey, id]);
  }

  return { saved: true, configured: true, count: items.length, deleted: deleteIds.length, savedAt: new Date().toISOString() };
}

async function loadSchoolRecordsMirror(accountKey) {
  const config = registryConfig();
  if (!config.url || !config.token) return [];
  const index = await registryRequest(config, ["smembers", recordsMirrorIndexKey(config, accountKey)]);
