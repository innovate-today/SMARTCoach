...
function getGhlContext(req) {
  ...
  return {
    ...
    parentEmailCoachAccess: normalizeParentEmailCoachAccess(account && account.parentEmailCoachAccess, account && account.coachSeats),
...
function normalizeSubscriptionStatus(value) {
  ...
  return ["active", "trialing", "past_due", "paused", "canceled", "incomplete", "incomplete_expired", "unpaid"].includes(status) ? status : "";
}
...
function coachCodeAllowed(req, providedCode) {
  const { accountKey, productPlan, accessCode, coachSeats, coachAccessCodes, parentEmailCoachAccess, requireCoachAccess, subscription } = getGhlContext(req);
  if (productPlan === "essential") {
  ...
  if (!allowedCodes.length) {
    ...
    return { allowed: true, accountKey, productPlan, coachSeats, coachIndex: 0, parentEmailAllowed: false };
  }
  const provided = cleanText(providedCode);
  const coachIndex = provided ? allowedCodes.findIndex((code) => safeEqual(provided, code)) : -1;
  if (coachIndex >= 0) return { allowed: true, accountKey, productPlan, coachSeats, coachIndex, parentEmailAllowed: !!parentEmailCoachAccess[coachIndex] };
  return {
    allowed: false,
    statusCode: 401,
    error: "SMART Trak coach access code is required.",
