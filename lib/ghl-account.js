...
function requireProPlan(req, res) {
  const { accountKey, productPlan, accessCode, coachSeats, coachAccessCodes, requireCoachAccess, subscription, coachCodeVersion } = getGhlContext(req);
  ...
  if (session && coachSessionVersionAllowed(session, coachCodeVersion)) return true;
