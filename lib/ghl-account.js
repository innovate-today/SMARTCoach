...
function requireProPlan(req, res) {
  const { accountKey, productPlan, accessCode, coachSeats, coachAccessCodes, subscription } = getGhlContext(req);
...
function coachCodeAllowed(req, providedCode) {
  ...
  const allowedCodes = coachAccessCodes && coachAccessCodes.length ? coachAccessCodes : accessCode ? [accessCode] : [];
  if (!allowedCodes.length) return { allowed: true, accountKey, productPlan, coachSeats };
...
function createCoachSession(accountKey, options = {}) {
  ...
  const ttlSeconds = Math.max(300, Math.min(Number(options.ttlSeconds) || 12 * 60 * 60, 7 * 24 * 60 * 60));
