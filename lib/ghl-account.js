...
function getGhlContext(req) {
  ...
  return {
    ...
    accountAccess: normalizeAccountAccess(account),
...
function defaultAccount() {
  return {
    ...
    accessStatus: process.env.SMARTCOACH_ACCESS_STATUS,
    accessReason: process.env.SMARTCOACH_ACCESS_REASON,
    betaExpiresAt: process.env.SMARTCOACH_BETA_EXPIRES_AT,
...
function accountFromKey(accountKey) {
  ...
  return {
    ...
    accessStatus: process.env[`SMARTCOACH_ACCESS_STATUS_${suffix}`],
    accessReason: process.env[`SMARTCOACH_ACCESS_REASON_${suffix}`],
    betaExpiresAt: process.env[`SMARTCOACH_BETA_EXPIRES_AT_${suffix}`],
...
function accountFromJson(accountKey) {
  ...
  try {
    ...
    return {
      ...
      accessStatus: account.accessStatus || account.accountAccessStatus,
      accessReason: account.accessReason || account.accountAccessReason,
      betaExpiresAt: account.betaExpiresAt || account.betaAccessExpiresAt,
...
function normalizeAccountAccess(account) {
  const source = account || {};
  const access = source.accountAccess || {};
  const betaExpiresAt = cleanText(source.betaExpiresAt || source.betaAccessExpiresAt || access.betaExpiresAt || access.betaAccessExpiresAt);
  const savedStatus = normalizeAccountAccessStatus(source.accessStatus || source.accountAccessStatus || source.status || access.status);
  const expired = betaAccessExpired(betaExpiresAt);
  return {
    status: expired ? "beta_expired" : savedStatus,
    savedStatus,
    active: savedStatus === "active" && !expired,
    reason: cleanText(source.accessReason || source.accountAccessReason || access.reason),
    betaExpiresAt,
    inactiveAt: cleanText(source.inactiveAt || access.inactiveAt),
    updatedAt: cleanText(source.accessUpdatedAt || access.updatedAt),
  };
}

function normalizeAccountAccessStatus(value) {
  ...
  const aliases = {
    ...
    disabled: "inactive",
    blocked: "inactive",
    suspended: "manual_hold",
    hold: "manual_hold",
    manual: "manual_hold",
    beta_expire: "beta_expired",
    expired: "beta_expired",
  };
  const normalized = aliases[status] || status;
  return ["active", "inactive", "manual_hold", "beta_expired"].includes(normalized) ? normalized : "active";
}

function betaAccessExpired(value) {
...
function requireProPlan(req, res) {
  const { accountKey, productPlan, accessCode, coachSeats, coachAccessCodes, coachStaff, requireCoachAccess, subscription, accountAccess, coachCodeVersion } = getGhlContext(req);
  ...
  if (!accountAccessAllowed(accountAccess)) {
    res.status(403).json({
      ...
      accountKey,
      productPlan,
      accountAccessStatus: accountAccess && accountAccess.status,
      accountAccessRequired: true,
    });
    return false;
  }
  if (req && req.smartcoachDeviceRevoked) {
    res.status(401).json({
      error: "This device session was revoked. Sign in again to continue.",
      accountKey,
      deviceRevoked: true,
...
function coachCodeAllowed(req, providedCode) {
  const { accountKey, productPlan, accessCode, coachSeats, coachAccessCodes, parentEmailCoachAccess, requireCoachAccess, subscription, accountAccess, coachCodeVersion } = getGhlContext(req);
  if (!subscriptionAccessAllowed(subscription)) {
    return {
      allowed: false,
      statusCode: 402,
...
function createCoachSession(accountKey, options = {}) {
  ...
  const payload = {
    ...
    sessionScope: cleanText(options.sessionScope),
    deviceId: cleanText(options.deviceId).slice(0, 160),
    deviceLabel: cleanText(options.deviceLabel).slice(0, 120),
    exp: expiresAt,
...
function coachSessionFromRequest(req, accountKey) {
  if (req && req.smartcoachDeviceRevoked) return null;
...
function accountAccessAllowed(accountAccess) {
  return normalizeAccountAccess(accountAccess).active;
}

function accountAccessBlockedMessage(accountAccess) {
  const access = normalizeAccountAccess(accountAccess);
  if (access.status === "beta_expired") return "SMART Trak access is blocked because beta access has expired.";
  if (access.status === "manual_hold") return "SMART Trak access is blocked because this account is on manual hold.";
  return "SMART Trak access is blocked because this account is inactive.";
}
