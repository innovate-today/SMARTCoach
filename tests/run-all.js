...
function checkCurrentFitnessClear() {
  ...
  [
    ...
    "async function deleteReplacedAthleteBest",
  ...
  ["plan-setup.html", "plan-builder.html"].forEach((file) => {
    ...
    [
      ...
      "data-original-event",
      "data-original-record-id",
      "previousEvent:row.getAttribute('data-original-event')||''",
      "previousRecordId:row.getAttribute('data-original-record-id')||''",
      "previousEvent:draft.previousEvent||''",
      "previousRecordId:draft.previousRecordId||''",
      "var hasDraftFitness=hasFitnessDraftTime();",
      "hasDraftFitness?'No current fitness changes to save.'",
      "function fitnessDraftChanged(athlete,draft)",
      "if(!saved)return true;",
      "previousEvent&&normalizeFitnessEvent(previousEvent)!==normalizeFitnessEvent(draft.event)",
      "fitnessValueChanged(draft.time,saved.resultDisplay)",
      "fitnessDateOnly(saved.resultDate)",
      "function applyFitnessSaveResults",
...
function checkAthleteRosterImportParentFields() {
  ...
  [
    ...
    "function importBareParentNameHeader",
    "groupName:importValue",
    "function syncImportedGroup",
    "friendlyApiError(error)",
    "id=\"importCsvFile\" type=\"file\" accept=\".csv,text/csv\"",
    "function loadImportCsvFile",
    "new FileReader()",
    "reader.readAsText(file)",
    "els.importCsvFile.addEventListener('change',loadImportCsvFile)",
    "function importCsvLines",
    "var lines=importCsvLines(text);",
    "Paste roster rows or upload a CSV with a header row before importing.",
    "function importValueMatching",
    "value==='g'",
    "function athleteFromSaveResult(existing,payload,result)",
    "var athlete=athleteFromSaveResult({},payload,result);",
    "var saved=athleteFromSaveResult(existing,payload,result);",
    "saved.contactId=saved.contactId||saved.id;",
    "student gender",
    "mother email",
    "father email",
  ...
  [
    ...
    "saveAthleteRosterDetail({ accountKey, athlete: normalized, payload: detailPayload })",
...
function checkDashboardActivityRangeLayout() {
  ...
  const required = [
    ...
    "recentAttendanceRows=Array.isArray(data.attendance)?data.attendance:[]",
    ...
    ".page{max-width:1480px;margin:0 auto;padding:20px 18px 132px}",
    "body.embedded .page{max-width:none;margin:0;padding:16px 18px 132px}",
    ".page{padding:14px 10px 132px}",
...
function checkDashboardFilterContextAndArchivedGroups() {
  ...
  [
    ...
    "rows=sortRows(seasonScopedRosterRows(rows,completedRows),tableSorts.roster,rosterSortValue);",
    "function seasonScopedRosterRows(rows,completedRows)",
    "if(value==='all')return rows;",
    "if(contact)keys['id:'+contact]=true;",
    ...
    "if(type==='training'&&legacyCrossCountryTrainingRow(row))return 'cross_country';",
    "function legacyCrossCountryTrainingRow(row)",
    "function legacyMileageDateBucketSeason(value)",
    "if(!completedVolumeMiles(row))return false;",
...
function checkCurrentFitnessCleanupTool() {
  ...
  [
    ...
    "function fitnessCleanupCurrentMatchesRace(current,race)",
    "Math.abs(currentMs-raceMs)<=5000",
    "function fitnessCleanupDateKey(value)",
    "currentMatchesLatest&&previous?fitnessCleanupRaceFitness(previous):current",
    "function fitnessCleanupRaceFitness(race)",
    "function fitnessCleanupProjectionSourceLabel(source,sourceKind)",
    "From previous race",
    "From current mark",
    "sourceLabel",
    "previous-race projection",
    "current-mark projection",
    "Projected",
    "Restore Previous",
    "Use Latest",
...
function checkMilesBoardFeature() {
  ...
  [
    ...
    "if(!hasBoardRows(data)&&restoreCachedBoard())return;",
    "writeCachedBoard(data);",
    "if(restoreCachedBoard())return;",
    "Last good update ",
...
function checkResultsBoardFeature() {
  ...
  [
    ...
    'id="resultsBoardCopyDisplay"',
    'id="resultsBoardOpenDisplay"',
    'id="resultsBoardDetailList"',
    'id="resultsBoardDetailsDefault"',
    "function resultsBoardDisplayUrl(url,displayMode)",
    "function resultsBoardPreferredShareUrl(data,displayMode)",
    "function copyResultsBoardDisplayLink()",
    "function openResultsBoardDisplayLink()",
    ...
    "function createResultsBoardLink(displayMode)",
    "compareValues(b.meetDate||'',a.meetDate||'')",
    "compareValues(b.syncedAt||'',a.syncedAt||'')",
  ...
  [
    ...
    "All Meets",
    'id="topResultCards"',
    "function renderTopResultCards",
    "summary.topResultCards",
    "Top Result - '+label",
    ...
    "var restored=forceRefresh?false:restoreCachedBoard();",
    "fetchWithTimeout(apiUrl(forceRefresh),{cache:'no-store'},restored?9000:18000)",
    "els.refreshBtn.addEventListener('click',function(){loadBoard({forceRefresh:true})});",
    "if(!hasBoardRows(data)&&restoreCachedBoard())return;",
    "if(restoreCachedBoard())return;",
    "Last good update ",
    ...
    "View link copied.",
    "setParam(p,'q',els.search.value)",
    "if(isDisplayMode())p.set('display','1');else p.delete('display');",
  ...
  [
    ...
    "class=\"sectiontitle\"",
    "class=\"athletehead\"",
    "function athleteBestRaceMs(section,athlete,races)",
    "var bestMs=athleteBestRaceMs(section,athlete,races);",
    "class=\"bestmark\">FAST</span>",
    "var bestClass=bestMs&&resultMs===bestMs?' final besttime':' final';",
    "function meetOptionHtml(value,label,checked)",
    "function updateMeetPickerLabel()",
    "function viewParams()",
    "function shareCurrentView()",
    "View link copied.",
    "function boardCacheKey()",
    ...
    "if(restoreCachedBoard())return;",
    "Last good update ",
    "function distanceGroupLabel(value)",
    "function gradeTitle(value,seasonYear)",
    "function rowGradeValue(row)",
    "function sectionKey(row){return[distanceGroupLabel(row.event),genderLabel(row.athleteGender)].join(' | ')}",
    "function raceKey(row){return[String(row.meetName||''),String(row.seasonYear||''),distanceGroupLabel(row.event),genderLabel(row.athleteGender)].join('|')}",
    "p.set('meet','__all__');p.delete('event');p.delete('gender');p.delete('grade');",
    "selectedMeetNames.forEach(function(name){p.append('meet',name)});else p.set('meet','__all__');",
    "if(selectedMeetNames.length&&selectedMeetNames.indexOf(row.meetName)<0)return false;",
    "else if(row.meetDate&&(!section.races[rk].date||String(row.meetDate)<String(section.races[rk].date)))section.races[rk].date=row.meetDate;",
    "function betterRaceRow(next,current)",
    "section.matrix[ak][rk]=betterRaceRow(row,section.matrix[ak][rk]);",
    "title:distanceGroupLabel(row.event)+' '+genderLabel(row.athleteGender)",
    "function render()",
  ...
  [
    ...
    "recordMatchesAthlete(record, athlete) && !isVoidedMeetResult(record)",
    ...
    "value.name || value.label || value.key || value.id || value.contactId || value.contact_id || value._id",
    "value.map(clean).filter(Boolean).join(\", \")",
    "valueContainsToken(contactValue, athlete.id)",
    "nameValue === normalizedName(athlete.name)",
    ...
    "function resultsBoardBestKey(row)",
    ...
    "function resultsBoardResultAtLeastBest(row, display, ms)",
    ...
    "resultsBetter(fallbackBest, { resultDisplay: next.bestDisplay, resultMs: next.bestMs })",
    "const seasonBest = seasonBests.get(resultsBoardSeasonBestKey(row));",
    "bestDisplay: clean(row.bestDisplay)",
...
function checkXcTop20RecordsFeature() {
  ...
  [
    ...
    'data-xc-list="boys2Mile"',
    'data-xc-list="girls2Mile"',
    'id="xcAddListSelect"',
    'id="xcAddListBtn"',
    'id="xcDeleteListBtn"',
    "var XC_DEFAULT_LIST_KEYS=['boys5k','girls5k','boys2Mile','girls2Mile'];",
    "var XC_FALLBACK_LIST_OPTIONS=[",
    "{key:'boys2Mile',label:'Boys 2 Mile',gender:'Boys',event:'2 Mile',defaultList:true}",
    "function normalizeXcCustomListKeys(keys)",
    "function xcVisibleListKeys()",
    "function xcListHasMarks(key)",
    "function renderXcListChoices()",
    "function addXcCustomList()",
    "function removeXcCustomList(key)",
    "function deleteActiveXcList()",
    "Show another XC list",
    "Choose extra XC distances with saved marks.",
    "No hidden lists with saved marks",
    "els.xcDeleteListBtn.disabled=XC_DEFAULT_LIST_KEYS.indexOf(xcActiveList)>=0;",
    'id="xcImportFile"',
    'id="xcImportText"',
    'id="xcEditModal"',
    'id="xcShareModal"',
    ...
    "function loadXcTop20View()",
    "function saveXcImport()",
    "function xcImportVisibilityMessage(rows,created,skipped)",
    "function xcAthleteNameKey(value)",
    "behind that athlete's faster saved mark",
    "outside the current Top 20",
    "Saved '+created+' XC mark'+(created===1?'':'s')+' to Meet History. Refreshing XC Top 20...",
    "function xcNormalizeEvent",
    "function xcGradeValue(value,seasonYear)",
    "grade:xcGradeValue(values.grade,seasonYear)",
    "function xcImportEventValue(value,defaultList)",
    "Original Event: '+row.sourceEvent",
    "event:row.event",
    "Grade can be 6, 7, 8, 9, 10, 11, or 12.",
    "if(/\\b(4\\s*k|4000\\s*m?)\\b/.test(key))return '4K';",
    "if(/\\b(3\\s*k|3000\\s*m?)\\b/.test(key))return '3K';",
    "if(gender==='Girls'&&/\\b(cross country|cc)\\b/.test(key))return '2 Mile';",
    "List fallback",
    "Meet fallback",
    "Year fallback",
    "Use these only when pasted rows leave blanks. List fallback fills missing gender and distance.",
    "Imported athletes do not need to be on the current roster.",
    "<th>Grade</th>",
    "Athlete, Time, Meet, Year, Event, Gender, Grade",
    "<th>Gender</th><th>Grade</th><th>Event</th>",
    "grade:row.grade",
    "rawYear",
    ...
    "if(text.indexOf('|')>=0)",
    "col.replace(/\\*\\*/g,'').trim()",
    "function isRecordSeparatorRow(cols)",
    "if(isRecordSeparatorRow(cols))return null;",
    "return rows.filter(function(row){return !row.athleteName||!row.resultDisplay||!row.event;})",
    "els.xcImportText.addEventListener('input',function(){",
    "previewXcImport();",
    "function refreshRecordsPage()",
    "var recordsRenderedFromCache=false;",
    "function renderCachedRecordsFirst()",
    "loadRecords({allowCachedFirst:firstRecordsLoad})",
    "Refresh failed. Showing saved records.",
  ...
  [
    ...
    "schemaKey: MEET_RESULT_SCHEMA_KEY, timeoutMs: meetRecordTimeoutMs",
    "schemaKey: ATHLETE_BEST_SCHEMA_KEY, timeoutMs: optionalRecordTimeoutMs",
    "schemaKey: PERFORMANCE_RECORD_SCHEMA_KEY, timeoutMs: optionalRecordTimeoutMs",
  ...
  [
    ...
    '["summer", "fall", "winter", "spring"].includes(normalized)',
...
function checkSpeedTrakFeature() {
  ...
  [
    ...
    "function loadSpeedAthletes(seq)",
    "apiUrl('/api/smart-trak/field-practice',{kind:'speed'})",
    "Roster Status",
    "rosterStatus",
    ...
    "function rosterAthleteForRep(rep,map)",
    "rep&&rep.smartcoachAthleteId,rep&&rep.contactId,rep&&rep.athleteId",
    'id="leaderboardTab"',
    ...
    "new Set(reps.map(leaderboardAthleteKey)).size",
    "function renderSpeedSessions()",
    "function openSpeedSession(index)",
    "function speedSessionDetailsHtml(session)",
    "openSessionId:''",
    "state.openSessionId=state.openSessionId===session.id?'':session.id",
    "open?'Close':'Open'",
    "session-detail-row",
    ...
    "data-session-edit",
    "data-session-delete",
    'id="speedSessionModal"',
    "data-edit-session",
    "data-delete-session",
    'id="deleteSpeedSessionModal"',
    "function openSpeedSessionModal(session)",
    "function saveSpeedSession()",
    "function deleteSpeedSession()",
    "body:JSON.stringify({deleteIds:[session.id]})",
    "Every athlete rep in this session will be removed.",
    "Changes apply to every athlete and rep in this session.",
    "practice.speedMetrics=(practice.speedMetrics||[]).map",
    "next.surface=surface",
    "next.timingMethod=timingMethod",
    "next.speedFocus=speedFocus",
    "next.startType=startType",
    "openSpeedResultModal(session.rows",
    "openDeleteSpeedResultModal(session.rows",
    "function buildAthleteProgressionRows()",
    "function renderAthleteProgression()",
    "'Choose metric'",
    "if(athlete&&metrics.length===1&&!els.progressionMetric.value)els.progressionMetric.value=metrics[0]",
    "if(!athlete||!metric)return[]",
    "row.athlete===athlete&&row.metric===metric",
    "Choose one speed metric for a valid like-for-like comparison.",
    "session.newPb=true",
    "function setSpeedTrakView(view)",
    'id="exportSpeedDataBtn"',
    "function exportSpeedData()",
    "function speedCsvCell(value)",
    "state.rows.slice().sort",
    "'Velocity (m/s)','MPH'",
    "a.download='speed-trak-data-'",
    "els.exportSpeedDataBtn.addEventListener('click',exportSpeedData)",
    'id="importTab"',
    ">Import Data</button>",
    ...
    'id="speedDistanceInput"',
    'id="speedDistanceUnitInput"',
    'id="speedFlyZoneInput"',
    'id="speedSurfaceInput"',
    'id="speedTimingInput"',
    'id="speedFocusInput"',
    'id="speedStartInput"',
    'id="speedSurfaceOtherInput"',
    'id="sessionSurfaceOtherInput"',
    '<option value="Indoors">Indoors</option>',
    '<option value="Other">Other</option>',
    "function surfaceFromEditor(select,input)",
    "surface:surfaceFromEditor(els.speedSurfaceInput,els.speedSurfaceOtherInput)",
    "surface=surfaceFromEditor(els.sessionSurfaceInput,els.sessionSurfaceOtherInput)",
    "if(text==='indoor'||text==='indoors')return'Indoors'",
    "timingMethod:normalizeTimingMethod(els.speedTimingInput.value)",
    "speedMetricDistance:metric.timedDistance||metric.distance||0",
    ...
    "function historicalGraduationYear(source)",
    "historicalGraduationYear(source)?'Historical':''",
    "function leaderboardAthleteKey(row)",
    "function bestLeaderboardRows(rows)",
    "filtered=bestLeaderboardRows(filtered)",
    "row.seconds===current.seconds&&String(row.date)>String(current.date)",
    "Athletes ranked",
    'id="surfaceSelect"',
    'id="timingSelect"',
    'id="focusSelect"',
    'id="startSelect"',
    "function populateFilterSelect(select,values,allLabel)",
    ...
    "data-speed-edit",
    "data-speed-delete",
    "state.practices=data.practices||data.items||[]",
  ...
  [
    ...
    "setBoardFilterParam(p,'surface',els.surfaceSelect.value)",
    "setBoardFilterParam(p,'timingMethod',els.timingSelect.value)",
    "setBoardFilterParam(p,'speedFocus',els.focusSelect.value)",
    "setBoardFilterParam(p,'startType',els.startSelect.value)",
    "Display Mode",
    "Challenge Highlights",
    "Velocity Leader",
    "Fastest Time",
  ...
  [
    ...
    "rep.athleteKey || identifiedByName.get(nameKey) || nameKey",
  ...
  [
    ...
    "if(!reps&&row&&row.speedMetricSession&&num(row.speedRepCount)>1)",
    "if(Math.abs(meters-num(row.completedVolumeMeters))<.01)return null;",
    ...
    "function speedMetricExplicitMeters(rep,practice)",
    "function shouldUseExplicitSpeedMeters(explicit,meters,source)",
    "if(explicit<=200&&meters>200)return true;",
    "distance:roundSpeedMetric(metric.unit==='yd'?meters/.9144:meters)",
...
function checkPowerTrakFeature() {
  ...
  [
    ...
    "bar.querySelector('[data-rack-device-check]').addEventListener('click',function(){runPowerRackDeviceCheck()})",
    "setTimeout(function(){renderPowerRackDeviceCheckSummary()},0)",
    "window.showPowerRackLogin=showRackLogin",
    "if(typeof window.showPowerRackLogin==='function')window.showPowerRackLogin();else location.reload()",
    "data-change-rack-login",
    "data-rack-device-check",
    'id="rackDeviceCheckModal"',
    "function runPowerRackDeviceCheck()",
    "function powerRackDeviceCheckKey()",
    "function renderPowerRackDeviceCheckSummary()",
    "function powerRackDeviceCheckFresh()",
    "Date.now()-checkedMs<12*60*60*1000",
    "Ready checked ",
    "Device check expired",
    "function startRackAfterReadiness(workout,selected)",
    "title:'Device check needed'",
    "does not have a current passing Device Check",
    "accept:'Start Anyway'",
    ...
    "deviceSource:'power-rack'",
    "X-SMARTCoach-Device-Source",
    ...
    ".rack-kiosk .rack-safety-actions{flex:0 0 auto",
    "Build strength workouts and review saved power testing sessions.",
    'data-power-tab="workouts"',
    'data-power-tab="rack"',
    'data-power-tab="sessions"',
    'data-power-tab="leaderboard"',
    'data-power-tab="progression"',
    'data-power-tab="import"',
    'data-power-tab="history"',
    'id="workoutList"',
    'id="workoutBlocks"',
    'id="powerExerciseCatalog"',
    'id="saveWorkoutBtn"',
    'id="archiveWorkoutBtn"',
    'id="addBlockBtn"',
    "Percentage of 1RM",
    "Completion only",
    "function blankPowerWorkout()",
    "function renderWorkoutBuilder()",
    "Rest after exercise (sec)",
    "Coaching cue",
    "Use RIR target",
    "Workout guidance only",
    "Target RIR",
    "function renderRirConfig(exercise,prefix)",
    'id="rackActualReps"',
    "Complete Set",
    "Saving Set...",
    "Undo Last Set",
    ...
    "prescribedReps:prescribedExecutionReps(position.exercise)",
    "data-key=\"executionMode\"",
    "leftReps:actual.leftReps,rightReps:actual.rightReps",
    "actualReps:actual.reps",
    'id="rackRepEditReps"',
    'data-key="instructions"',
    "exercise&&exercise.instructions",
    'data-key="restSeconds"',
    "function powerResultWithRest(result,exercise)",
    ...
    "function rackPrescribedRepTotal(workout)",
    "function rackMonitorProgress(rack)",
    "function rackUpdatedLabel(rack)",
    "rack.deviceLabel||'Rack device'",
    "rack-update-stale",
    ...
    "function powerRackPreferenceKey(name)",
    "function powerRackDefaultName()",
    "rememberPowerRackPreference('workout',workout.id)",
    "label.replace(/\\s*iPad\\s*$/i,'').trim()",
    "item.status==='active'&&item.deviceId===deviceId",
    "Active rack restored on this iPad.",
    ...
    'id="rackSyncStatus"',
    "data-retry-rack-sync",
    "function requestPowerRackLoginChange()",
    "title:'Change rack login?'",
    "These updates belong to this device and account.",
    "claim-rack-athlete",
    "release-rack-athlete",
    "setInterval(refreshRackReservations,5000)",
    "Reserved on this iPad",
    "Reserved on '+reservation.deviceLabel",
    "function activeRackAthletes(rack)",
    'id="provisionalAthleteModal"',
    "function saveProvisionalAthlete()",
    "power_provisional_",
    "Needs Roster Review",
    "provisionalAthlete:athlete,deviceId:powerRackDeviceId()",
    'id="reviewProvisionalBtn"',
    "function approveProvisionalAthlete(id)",
    "smartcoachAthleteId:athlete.id",
    "Approve to Roster",
    "function releaseActiveRackAthlete()",
    'id="rackCompletionModal"',
    "function showRackCompletion(rack)",
    "function finishRackView(rack)",
    "Stopped early",
    "All rack data synced",
    "athlete.rackStatus='complete'",
    "if(!activeRackAthletes(rack).length){rack.status='complete';rack.completedAt=now.toISOString()}",
    "completed the workout.",
    "data-release-rack-athlete",
    "Completed reps remain in this session.",
    "blocked?'disabled':''",
    ...
    "data-skip-rack-rest",
    "athlete.restUntil=startsRest&&!finished?",
    "renderRackLiveCore=renderRackLive",
    "function powerExerciseNames()",
    "function renderPowerExerciseCatalog()",
    'id="manageExercisesBtn"',
    'id="exerciseLibraryModal"',
    "function saveExerciseCatalog(message)",
    "exerciseCatalog:names",
    'data-clear-exercise=',
    'list="powerExerciseCatalog"',
    "replace(/\\b(barbell|dumbbell|1rm|one rep max|one-rep max)\\b/g,'')",
    "function savePowerWorkout()",
    "function deletePowerWorkout()",
    "function powerConfirm(options)",
    'id="powerConfirmModal"',
    "function togglePowerWorkoutArchive()",
    "state.workoutDraft&&state.workoutDraft.status==='archived'?'Restore':'Archive'",
    "state.workouts.filter(function(workout){return workout.status!=='archived'})",
    "function powerOneRepMaxKey(value)",
    "function powerOneRepMax(athlete,exerciseName,unit)",
    "function rackEntryDefault(athlete,exercise,prescribed)",
    "function rackLoadStep(unit)",
    "function adjustRackLoad(direction)",
    "data-adjust-rack-load",
    "Decrease load by ",
    "Increase load by ",
    "['fixed_load','percent_1rm','athlete_entry']",
    "result.exerciseId===exercise.id",
    "/1rm|one.?rep.?max/i.test(result.exerciseName||'')",
    "increment=unit==='kg'?2.5:5",
    "percent+'% of '+max.value+' '+unit+' 1RM'",
    'id="rackWorkout"',
    'id="rackAthletePicker"',
    'id="startRackBtn"',
    'id="lateAthleteSelect"',
    ...
    "Enter actual reps from 1 to 1000 before completing this set.",
    "Enter a valid weight or result before completing this set.",
    "Enter the result unit before completing this set.",
    "if(!actual)return",
    'input[aria-invalid="true"]',
    "var rackRepSavePending=false",
    "if(rackRepSavePending)return",
    "rackRepSavePending?'Saving Set...'",
    ".finally(function(){rackRepSavePending=false",
    "function undoRackRep()",
    "function addLateRackAthlete()",
    'id="rackSessionRows"',
    'id="exportRackSessionsBtn"',
    'id="exportRackCompletionBtn"',
    'id="exportRackWorkloadBtn"',
    'id="rackSessionStatus"',
    'id="rackSessionCompletion"',
    'id="rackSessionWorkout"',
    "function renderRackSessions()",
    "function filteredRackSessions()",
    "function exportRackSessions()",
    "function exportRackCompletion()",
    "function exportRackWorkload()",
    "power-trak-exercise-workload-",
    "'Average Load Per Rep','Heaviest Rep'",
    "'Completion Percent'",
    "power-trak-completion-",
    "'Set','Prescribed Reps','Actual Reps','Execution','Left Reps','Right Reps','Prescribed Value','Actual Value','Unit','Completed At'",
    "power-trak-rack-results-",
    "function rackSessionDetail(rack)",
    "function saveRackRepCorrection(rack,message)",
    "function editRackSession(rack)",
    "data-edit-rack-session",
    'id="rackSessionEditDate" type="date"',
    "Choose a valid workout date.",
    "function correctRackRep(button,remove)",
    "data-edit-rack-rep",
    "data-delete-rack-rep",
    "This cannot be undone.",
    "function deleteRackSession(rack)",
    "Delete '+rack.rackName+' and all recorded athlete reps?",
    'id="powerLeaderboardExercise"',
    'id="powerLeaderboardUnit"',
    'id="powerLeaderboardYear"',
    'id="powerLeaderboardLimit"',
    'id="powerLeaderboardRows"',
    "Best comparable result per athlete for coach-approved exercises.",
    'data-leaderboard-board="strength"',
    'data-leaderboard-board="explosive"',
    'id="powerLeaderboardReps"',
    'id="powerLeaderboardManage"',
    "function leaderboardSetResults()",
    "function leaderboardUnitKind(unit)",
    "save-leaderboard-exercises",
    "function powerLeaderboardResults()",
    "function renderPowerLeaderboard()",
    "row.value>prior.value",
    'id="powerProgressionAthlete"',
    'id="powerProgressionExercise"',
    'id="powerProgressionUnit"',
    'id="powerProgressionYear"',
    'id="powerProgressionChart"',
    'id="powerProgressionRows"',
    "Compare one athlete's measurable results for the same exercise and unit.",
    "function powerProgressionChart(sessions,lowerIsBetter,unit)",
    "function renderPowerProgression()",
    "lowerIsBetter?sessions[0].best-latest.best:latest.best-sessions[0].best",
    "Completion-only exercises do not appear.",
    'id="powerImportFile"',
    'id="powerImportUnit"',
    'id="powerImportText"',
    'id="powerImportPreviewBtn"',
    'id="powerImportSaveBtn"',
    'id="powerImportManageBtn"',
    'id="powerImportPreview"',
    "function parsePowerDelimited(text)",
    "function buildPowerImportPreview()",
    "function savePowerImport()",
    "function managePowerImports()",
    "action:'cleanup-power-import'",
    "name:exercise+' 1RM'",
    "name:exercise+' Volume Load'",
    "for(var start=0;start<athletes.length;start+=4)",
    "Reimporting the same rows updates these sessions without duplicates.",
    "JSON.stringify({rackSessions:preview.rackSessions})",
    "JSON.stringify({rackSession:rack})",
    "JSON.stringify({workout:workout,exerciseCatalog:names})",
    "JSON.stringify({deleteWorkoutIds:[workout.id]})",
    "id=\"groupSelect\"",
    "id=\"metricSelect\"",
    "id=\"sessionSelect\"",
    "id=\"downloadCsvBtn\"",
    ...
    "function accessStorageKey(){return 'sc_access_'+smartCoachAccountKey()}",
    "function sessionStorageKey(){return 'sc_session_'+smartCoachAccountKey()}",
    "function rememberedSessionStorageKey(){return 'sc_session_remembered_'+smartCoachAccountKey()}",
    "localStorage.getItem(rememberedSessionStorageKey())||sessionStorage.getItem(sessionStorageKey())",
    ...
    "var hay=[row.athlete,row.grade,row.gender,row.groupName,row.testName,row.metric,row.mark,row.note]",
    "row.athleteName=athlete||row.athleteName||target.athlete;",
    "row.grade=String(els.editMarkGrade.value||'').trim();",
    "row.gender=String(els.editMarkGender.value||'').trim();",
    "row.marks[target.metricKey]=mark;",
    "body:JSON.stringify({session:session})",
    "Saved '+row.athleteName+' '+target.metric+'.",
    "JSON.stringify({deleteIds:[session.id]})",
    "title:'Delete saved test?'",
    "and every athlete mark in this test?",
    "Rack workouts, rack sessions, and other saved tests will remain.",
    "Deleted '+label+'.",
    ...
    'id="rackSummaryReps"',
    'id="rackSummaryLoad"',
    'id="rackSummaryCompletion"',
    'id="rackAthleteCompletionRows"',
    'id="rackExerciseWorkloadRows"',
    'id="rackSessionTable"',
    "completion:completion",
    "Math.min(100,Math.round(comparable/prescribed*100))+'%'",
    "function rackAthleteSessionProgress(rack,athlete)",
    "function rackSessionCompletionState(rack)",
    "Fully completed",
    "Imported history",
    "Stopped early",
    "recorded reps",
    "function rackSessionSummary(racks)",
    "function rackResultLoad(result)",
    "function powerLoadUnit(value)",
    "['lb','lbs','pound','pounds']",
    "['kg','kgs','kilogram','kilograms']",
    "powerLoadUnit(result.unit)!==targetUnit",
    "function normalizedRackRepCorrection(result,value,unit)",
    'id="rackSessionEditModal"',
    "function validRackSessionDate(value)",
    "function saveRackSessionEdit()",
    "Choose a valid workout date.",
    'id="powerImportCleanupModal"',
    "function openPowerImportCleanup(cleanup)",
    "function deletePowerImports()",
    "Type the confirmation phrase exactly as shown.",
    'id="rackRepEditModal"',
    "function openRackRepEdit(rackId,athleteIndex,resultIndex)",
    "function saveRackRepEdit()",
    "title:'Delete saved set?'",
    "JSON.parse(JSON.stringify(rack))",
    "Enter a valid nonnegative result.",
    "Enter the result unit.",
    "powerLoadUnit(unitText)||unitText",
    "if(targetUnit==='lb')state.sessions.forEach",
    "function rackLoadLabel(loads)",
    "'Recorded Load LB','Recorded Load KG'",
    "function rackAthleteCompletionSummary(racks)",
    "function renderRackAthleteCompletion(racks)",
    "function viewRackAthleteSessions(name)",
    "function viewRackExerciseSessions(name)",
    "data-view-exercise-sessions",
    "searchable.push(result.exerciseName)",
    "Search rack, athlete, or exercise",
    "function rackExerciseWorkloadSummary(racks)",
    "function renderRackExerciseWorkload(racks)",
    "Actual load-bearing reps in current filtered sessions",
    "data-view-athlete-sessions",
    "View Sessions",
  ...
  [
    ...
    'Rack devices cannot change coach-managed Power Trak data.',
  ...
  [
    ...
    "Use **Workouts** to create reusable rack workouts.",
    "use **Add Provisional Athlete**",
    "marks them **Needs Roster Review**",
    "Use **Review Provisional** and **Approve to Roster**",
    "suggest names from the account's Exercise Library",
    "Use **Manage Exercises** to add, rename, or remove",
    "**Clear** keeps the row but empties its exercise fields",
    "optional rest seconds after that exercise",
    "an optional coaching cue",
    "prescribed rest countdown blocks only that athlete's next set",
    "**Archive** an old template to keep it in the Workout Library",
    "Choose whether athletes use a fixed load, a percentage of their saved one-rep max, an athlete-entered load, completion only, height, distance, or time.",
    "Percentage workouts automatically use the athlete's best matching one-rep max",
    "Use **Athlete Progression** to follow one athlete's results for the same exercise and unit over time.",
    "a saved set's actual reps, result, and unit can be corrected",
    "one CSV row per completed set",
    "Use **Edit Session** to correct the rack/session name or workout date",
    "tap **Complete Set** once",
...
function checkDashboardWhatsNew() {
  ...
  [
    ...
    ".start-here-btn{display:inline-flex;align-items:center;gap:6px;border:1px solid #b7c7ea;background:#eef6ff;color:#173891;border-radius:999px;font-size:12px;font-weight:900;line-height:1;padding:7px 10px;min-height:0;white-space:nowrap;flex:0 0 auto}",
...
function checkDashboardStaffAccessHandoff() {
  ...
  [
    ...
    "Use this private link to open SMART Trak:",
    "appOnly?'':options.inviteLink||desktopLink",
...
function checkStravaAdminTestFlow() {
  ...
  [
    ...
    "function stravaEnvStatusText(status)",
    "function stravaMissingEnvNames(status)",
    ...
    "Already imported for selected athlete.",
    "Already Imported",
    "renderStravaActivities(stravaActivitiesCache);",
    "This Strava activity is already imported for ",
    ...
    "response=json",
    "result.data.authUrl",
    'id="stravaTestTitle">Strava</h2>',
    "Strava is available from SMARTCoach Admin.",
    "Tokens stay hidden.",
    "ID '+(status.clientId?'present':'missing')",
    "Secret '+(status.clientSecret?'present':'missing')",
    "Redirect '+(status.redirectUri?'present':'missing')",
  ...
  [
    ...
    'href="/training-calendar.html#strava"',
    'id="stravaModal"',
    'id="stravaTitle">Strava</h2>',
    'id="stravaConnectBtn"',
    'id="stravaLoadActivitiesBtn"',
    ...
    "function betaAdminToolsVisible()",
    "function stravaBetaAdminVisible()",
    "document.querySelectorAll('[data-beta-admin-only]').forEach(function(el){el.hidden=!showBeta;});",
    "Owner/admin beta access is required to use Strava.",
    "function stravaAdminUrl()",
    "function openStravaModal()",
    "function loadStravaStatus()",
    "function refreshStravaStatusIfModalOpen()",
    "function startStravaStatusRefreshPoll()",
    "function stopStravaStatusRefreshPoll()",
    "function connectStravaAccount()",
    "function openStravaAthleteConnectLink()",
    "function createStravaAthleteConnectLink()",
    "function stravaConnectionForAthlete(athlete)",
    "function stravaAthleteRequestParams(row)",
    "function stravaConnectionNameMismatch(athlete,connection)",
    "function stravaConnectionMatchesStravaName(athlete,connection)",
    "function stravaConnectionDisplayName(connection)",
    "function loadStravaActivities()",
    "stravaActivitiesLoadedAthleteKey=athleteKey",
    "function loadStravaImportData()",
    "function importStravaActivity(activityId,button)",
    "function stravaPlanMatchForActivity(activity,athlete)",
    "function stravaCalendarDayMatchesAthlete(day,athlete)",
    "function stravaTrainingGroupHasAthlete(groupName,athlete)",
    "Matched plan: ",
    "No same-day calendar match; imports as Strava easy run.",
    "Refreshing SMART Trak roster for import...",
    "fetch('/api/smart-trak/dashboard?v='",
    "fetch('/api/smart-trak/sync-session'",
    "sourceSessionId:'strava_'+(activity.id||dateText)",
    "trainingPlanDayTitle:matchTitle||''",
    "trainingPlanDayWorkoutType:matchDay?stravaWorkoutTypeFromDay(matchDay):'Easy/Recovery Run'",
    "plannedVolume:matchDay&&matchDay.plannedVolume||''",
    "Matched calendar workout: '+matchTitle",
    "Already imported for selected athlete.",
    "Already Imported",
    "renderStravaActivities(stravaActivitiesCache);",
    "Load Athlete Activities again after changing athletes.",
    "stravaActivitiesLoadedAthleteKey=''",
    "stravaActivityDetailsCache={}",
    'data-strava-detail="',
    "Inspect Details",
    "function stravaActivityDetailHtml(activityId)",
    "function inspectStravaActivity(activityId,button)",
    "function loadStravaActivityDetail(activityId)",
    "fetch('/api/smart-trak/strava-athlete-activity-detail?account='",
    "Strava API detail:",
    "function stravaImportPayload(activity,athlete,detail)",
    "function stravaImportRunSummary(matchDay,activity,detail)",
    "function stravaStandardSplitSummaryFromDetail(detail)",
    "source:splitSummary?'standard_splits':'activity_summary'",
    "distanceMeters:splitSummary&&splitSummary.distanceMeters||stravaImportDistanceMeters(activity,detail)",
    "function stravaImportDistanceMeters(activity,detail)",
    "function stravaImportDurationSeconds(activity,detail)",
    "function stravaStreamTotalDistanceMeters(streams)",
    "function stravaImportLapsForActivity(matchDay,detail)",
    "function stravaPlannedLapsFromDay(day)",
    "function stravaLapsFromDetail(detail)",
    "function stravaStandardSplitsFromDetail(detail)",
    "Strava Mile '+(split.split||split.index||index+1)",
    "function stravaRepPaceNoteBlock(matchDay,detail)",
    "function stravaRepPaceRowsFromStreams(planned,streams)",
    "function stravaStreamDistanceAt(streams,seconds)",
    "function stravaRepPaceRowsFromLaps(planned,laps)",
    "function stravaActivityDetailNoteLines(detail)",
    "Strava API standard splits exposed:",
    "function stravaActivityDetailStandardSplitSummaryText(split)",
    "Imported rep/rest layout from matched SMART Trak workout.",
    "Strava rep pace details saved from Strava stream/lap data.",
    "Rep pace details:",
    "Strava API laps exposed:",
    "returnPath='/strava-connected.html?account='",
    "fetch('/api/smart-trak/strava-athlete-connections?account='",
    "fetch('/api/smart-trak/strava-athlete-start?'",
    "fetch('/api/smart-trak/strava-athlete-activities?account='",
    "params.set('athleteName',row&&row.name||'');",
    "function stravaActivityRequestParams(athlete)",
    "params.set('limit','30');",
    "function stravaActivityLookbackAfterUnix()",
    "Strava is connected, but Strava returned no recent activities for ",
    "Expected: read,activity:read,activity:read_all",
    "function stravaConnectionMissingPrivateActivityScope(connection)",
    "reconnect this athlete with the updated link",
    "function renderStravaEmptyDiagnostic(diagnostic,connection)",
    "registryWarning=result.data&&result.data.registryWarning||''",
    "els.stravaStatus.style.color=registryWarning?'var(--warn)':'var(--good)'",
    "'Registry note: '+diagnostic.registryWarning",
    "Returned rows: ",
    "'&'+athleteParams+'&id='+encodeURIComponent(activityId)+'&v='",
    "if(els.stravaTrainingLink)els.stravaTrainingLink.addEventListener('click',function(event){",
    "window.addEventListener('focus',refreshStravaStatusIfModalOpen);",
    "document.addEventListener('visibilitychange',function(){",
    "After the athlete connects, this window will refresh the Strava status.",
    "Complete Strava authorization, then return here; SMARTCoach will refresh the connection status.",
    "if(location.hash==='#strava'&&stravaBetaAdminVisible())setTimeout(openStravaModal,0);",
  ...
  [
    'route === "strava-status"',
    'route === "strava-start"',
    'route === "strava-athlete-connections"',
    'route === "strava-athlete-start"',
    'route === "strava-callback"',
    'route === "strava-activities"',
    'route === "strava-athlete-activities"',
    'route === "strava-activity-detail"',
    'route === "strava-athlete-activity-detail"',
    ...
    "function accountStravaActivityDetail(req, res)",
    "function accountStravaAthleteConnections(req, res)",
    "function accountStravaAthleteStart(req, res)",
    "function accountStravaAthleteActivities(req, res)",
    "diagnostic: {",
    "rawCount: Array.isArray(data) ? data.length : 0",
    "function accountStravaAthleteActivityDetail(req, res)",
    ...
    "This Strava login appears to be",
    "Please sign out of Strava or switch accounts",
    ...
    "stravaConnection",
    "stravaAthleteConnections",
    "function normalizeSavedStravaAthleteConnection(value)",
    "function sanitizeStravaAthleteConnections(value)",
    "function activeStravaAthleteConnection(accountKey, accountRecord, athleteKeyValue, requestedAthleteValue)",
    "function saveStravaMaintenanceRecord(accountKey, record, connection)",
    "registryWarning: connection.registryWarning || \"\"",
    "SMARTCoach loaded Strava, but could not save the refreshed Strava token.",
    "const requestedAthlete = stravaAthleteFromRequest(req);",
    "stravaConnectionNameMismatchForAthlete(strictConnection, requestedAthlete)",
    "stravaConnectionMatchesStravaAthleteName(item, requestedAthlete)",
...
function checkDashboardToolPreferences() {
  ...
  [
    ...
    "function dashboardAccountStatusCacheKey()",
    "function readDashboardAccountStatusCache()",
    ...
    "<img data-brand-logo alt=\"SMART Trak\" width=\"64\" height=\"64\" decoding=\"async\" fetchpriority=\"low\">",
    ...
    "var dashboardLoadRequest=null;",
    "if(dashboardLoadRequest&&!options.forceRefresh)return dashboardLoadRequest;",
    "dashboardLoadRequest=request;",
    "if(dashboardLoadRequest===request)dashboardLoadRequest=null;",
    "var dashboardLoadSequence=0;",
    "var dashboardLiveRefreshSequence=0;",
    "var dashboardLiveRefreshRequest=null;",
    "var dashboardLiveLastLoadedAt=0;",
    "var dashboardBackgroundControllers=[];",
    "var dashboardSecondaryRequest=null;",
    "var dashboardSecondaryLastLoadedAt=0;",
    "var DASHBOARD_SECONDARY_BROWSER_TTL_MS=120000;",
    "var loadSequence=++dashboardLoadSequence;",
    "abortDashboardBackgroundRequests();",
    "if(loadSequence!==dashboardLoadSequence)return Promise.resolve();",
    "if(loadSequence!==dashboardLoadSequence)return;",
    "function trackDashboardBackgroundController(controller)",
    "function releaseDashboardBackgroundController(controller)",
    "function abortDashboardBackgroundRequests()",
    "dashboardLiveRefreshRequest=null;",
    "dashboardSecondaryRequest=null;",
    "dashboardBackgroundControllers.forEach(function(controller)",
    "dashboardBackgroundControllers=[];",
    "function dashboardSupportFetch(url,fetchOptions,fallback,timeoutMs)",
    "controller=trackDashboardBackgroundController(new AbortController());",
    "finally(function(){",
    "releaseDashboardBackgroundController(controller);",
    "els.refreshBtn.addEventListener('click',function(){loadDashboard({forceRefresh:true});});",
    "function dashboardSnapshotAgeMs(data)",
    "function dashboardSnapshotIsFresh(data)",
    "dashboardSnapshotAgeMs(data)<120000",
    "function dashboardMeetRowsFromPayload(data,options)",
    "if(!options.forceRefresh&&!incoming.length&&recentMeetRows.length)return recentMeetRows;",
    "var DASHBOARD_VIEW_STATE_VERSION='2026-09-12-recent-meet-date-default';",
    "meet:{key:'meetDate',dir:'desc'}",
    "viewVersion:DASHBOARD_VIEW_STATE_VERSION",
    "tableSorts=mergeSorts(defaultTableSorts(),raw.sorts,raw.viewVersion);",
    "var migrateMeetSort=viewVersion!==DASHBOARD_VIEW_STATE_VERSION;",
    "if(key==='meet'&&migrateMeetSort)return;",
    "var existing=readDashboardBrowserSnapshot();",
    "if(previousMeetResults.length)snapshot.recentMeetResults=previousMeetResults;",
    "writeDashboardBrowserSnapshot(Object.assign({},data,{recentMeetResults:nextRecentMeetRows}));",
    "pruneDashboardBrowserSnapshotAfterVoid(request,keep);",
    "function pruneDashboardBrowserSnapshotAfterVoid(request,keep)",
    "data.recentMeetResults=(Array.isArray(data.recentMeetResults)?data.recentMeetResults:[]).filter(keep);",
    "data.recentTrainingSyncs=(Array.isArray(data.recentTrainingSyncs)?data.recentTrainingSyncs:[]).filter(keep);",
    "recentMeetRows=nextRecentMeetRows;",
    "var lastFitnessRefreshSignal=(function(){",
    "try{return localStorage.getItem(fitnessRefreshKey)||'';}catch(e){return '';}",
    "if(options.forceRefresh){",
    "return fetch(liveUrl+'&refresh=1',fetchOptions).then(function(res){return res.json().then(function(data){return{ok:res.ok,data:data};});});",
    "if(!dashboardSnapshotIsFresh(browserSnapshot))scheduleDashboardLiveRefresh(stamp,fetchOptions,options.loadSequence);",
    "if(!dashboardSnapshotIsFresh(result.data))scheduleDashboardLiveRefresh(stamp,fetchOptions,options.loadSequence);",
    "if(dashboardLiveRefreshRequest)return;",
    "Date.now()-dashboardLiveLastLoadedAt<30000",
    "var refreshSequence=++dashboardLiveRefreshSequence;",
    "if(dashboardLiveRefreshRequest)return dashboardLiveRefreshRequest;",
    "dashboardLiveLastLoadedAt=Date.now();",
    "dashboardLiveRefreshRequest=request;",
    "if(dashboardLiveRefreshRequest===request)dashboardLiveRefreshRequest=null;",
    ...
    "function dashboardSecondaryBrowserSnapshotKey()",
    "function readDashboardSecondaryBrowserSnapshot()",
    "Date.now()-savedAtMs>DASHBOARD_SECONDARY_BROWSER_TTL_MS",
    "function writeDashboardSecondaryBrowserSnapshot(data)",
    "function applyDashboardSecondaryPayload(data,baseTrainingRows)",
    "var cached=forceRefresh?null:readDashboardSecondaryBrowserSnapshot();",
    "applyDashboardSecondaryPayload(cached,baseTrainingRows);",
    "writeDashboardSecondaryBrowserSnapshot(payload);",
    "},150);",
  ...
  [
    ...
    "dashboardSecondaryLastLoadedAt=Date.now();",
    "dashboardSupportFetch('/api/smart-trak/groups?v='+stamp",
    "recentAttendanceRows=Array.isArray(data.attendance)?data.attendance:[];",
    "docuItems=Array.isArray(data.docuItems)?data.docuItems:[];",
...
function checkBugTrakDesktopFeedback() {
  ...
  [
    ...
    "new MutationObserver(function(){tuckSmartCoachChatBadge();})",
    "Feedback",
    "Bug Trak",
    "Idea Trak",
...
function checkCalendarMeetSportPropagation() {
  ...
  [
    ...
    "var runners=manualRoster?[]:meetRunnersForGroup(groupNames,log);",
...
function checkDashboardMeetCorrectionFields() {
  ...
  [
    ...
    'id="meetCorrectionAthleteWrap"',
    'id="meetCorrectionAthlete"',
    ...
    "meetCorrectionAthlete:document.getElementById('meetCorrectionAthlete')",
    ...
    "function populateMeetCorrectionAthleteOptions",
    "function selectedMeetCorrectionAthlete",
    "populateMeetCorrectionAthleteOptions(meetCorrectionRow)",
    "contactId:payloadContactId",
    "smartcoachAthleteId:payloadSmartcoachAthleteId",
    "athleteName:payloadAthleteName",
    ...
    "smartcoachAthleteId:row.smartcoachAthleteId||''",
    "sport:row.sport||''",
    "season:row.season||''",
  ...
  [
    ...
    "meetResultSnapshotFingerprint(row) === sourceFingerprint",
...
function checkTrainingCorrectionWorkoutNoteReplacement() {
  ...
  [
    ...
    "Strava activity id|Matched calendar workout|Matched group|Strava activity|Strava start|Strava type|Strava distance|Strava moving time|Strava API laps exposed|Strava API standard splits exposed|Strava Lap|Strava Mile|Strava streams note",
  ...
  [
    ...
    "function repPaceReviewHtml(row)",
    "function repPaceReviewRows(row)",
    "function repPaceTargetStatus(row)",
    "function targetRangeIsPace(text)",
    "if(targetRangeIsPace(row&&row.plannedTarget))target=null;",
    "if(targetRangeIsPace(row&&row.plannedTarget))return '';",
    "var paceStatus=repPaceTargetStatus(row);",
    "if(paceStatus)return paceStatus;",
    "'<div class=\"pace-review-title\"><span>Actual vs Target</span><span>'+esc(rows.length+' entries')+'</span></div>'+",
...
function checkTrainingCalendarQualityEditParsing() {
  ...
  const required = [
    ...
    "function parseQualityWorkItem(text)",
    ...
    "data-add-repeat-work",
    "data-repeat-items",
    "data-repeat-work",
    "Add Work Inside Repeat",
    "return block.kind==='repeat'?block.repeatCount+' x ('+inner+')':inner;",
    "(block.items||[]).forEach(function(item){total+=block.repeatCount*item.reps*unitMiles(item.distanceValue,item.distanceUnit);});",
    "distanceValue:match[2],",
    "distanceUnit:normalizeQualityDistanceUnit(match[3]),",
    "effort:String(match[4]||'Threshold').trim(),",
    ...
    "function qualityBlockEffort(block)",
    "var firstEffort=workBlocks.map(qualityBlockEffort).filter(Boolean)[0]||type||'Threshold';",
  ...
  if (html.includes("Recovery Between Sets") || html.includes("setRecoveryValue") || html.includes("qualitySetRecoveryLine")) {
    throw new Error("Training Calendar repeating quality sets should use separate rest sets instead of recovery between sets.");
  }
...
function checkManualMileageQualitySession() {
  ...
  [
    ...
    'id="correctionSeason"',
    "function correctionSeasonContext(row)",
...
function checkMobileCalendarWorkoutPriority() {
  ...
  [
    ...
    "changed=clearRemovedCalendarSelectionsFromLogs(days)||changed;",
    "function clearRemovedCalendarSelectionsFromLogs(days)",
    "var removed=(days||[]).filter(isRemovedPlanDay);",
    "if(matchesRemovedSelection(log)){",
    "if(matchesRemovedSelection(r)){",
...
function checkMobileMeetRaceDivisionSetup() {
  ...
  [
    ...
    "raceRosterManual:l.raceRosterManual?1:0",
    "function meetRosterGroupLabel(group)",
    "function raceRosterGroupOptions()",
    "function raceRosterGroupSetupHtml()",
    "function applyXCRaceRosterGroup()",
    ...
    "function meetGroupSetupHtml(forceTools)",
    "if(!forceTools)return '';",
    "(forceTools?raceRosterGroupSetupHtml():'')",
    "XC_TOOLS_SETUP_OPENED={}",
    "function currentMeetSetupKey()",
    "function shouldOpenXCToolsSetup()",
    "if(!isCrossCountryRaceMode()||(CL.runners||[]).length)return false;",
    "if(shouldOpenXCToolsSetup())setTimeout(openXCTools,60);",
    "function emptyXCRaceSetupHtml()",
    "Set up this XC race",
    "Choose division and athletes in XC Tools.",
    "Open XC Tools",
    "isCrossCountryRaceMode()?emptyXCRaceSetupHtml()",
...
function checkMobileMeetRaceSummary() {
  ...
  [
    ...
    "function raceSummarySnapshots()",
    "function raceSummaryAvailableCount()",
    "return raceSummaryDisplayRows().length+raceSummarySnapshots().reduce(function(sum,snapshot)",
    "function preserveRaceSummarySnapshot(reason)",
    "raceSummarySnapshots().reduce(function(sum,snapshot)",
    "CL.raceSummarySnapshots=CL.raceSummarySnapshots.slice(0,8);",
    "function trackMeetSavedRaceSummaryRows()",
    "filter(pastResultIsCurrentMeet).map(function(row)",
    "note:'Saved SMART Trak result'",
    "function raceSummaryDisplayRows()",
    "rows.concat(trackMeetSavedRaceSummaryRows()).filter(function(row)",
    "function raceSummaryHtml()",
    "Loading Current Meet Results...",
    "Previous Race Summaries",
    "No saved race results yet.",
    "function raceSummaryText()",
    "function openRaceSummary()",
    "loadPastResults(false).then(function()",
    "currentBody.innerHTML=raceSummaryHtml();",
    "function shareRaceSummary()",
    "function raceSummaryShortcutHtml(forceTools)",
    'class="mover h" id="m-past-results"',
    "Past Results",
    "id=\"past-results-search\"",
    "Current event only",
    "function pastResultsShortcutHtml(forceTools)",
    "function loadPastResults(force)",
    "function fetchPastResultsRows(path,timeoutMs)",
    "function pastResultsMergeRows(primary,secondary)",
    "function pastResultsEventRows()",
    "Current Event Results",
    "Recent Saved Results",
    "No saved Past Results loaded yet. Tap Refresh Past Results after the race has been saved.",
    "/api/smart-trak/dashboard?light=1&refresh=1",
    "/api/smart-trak/dashboard?meetHistory=1",
    "Showing recent results while full history loads...",
    "Showing recent results. Full history did not finish loading.",
    "function normalizePastResultRow(row)",
    "function pastResultsSplitRows(text)",
    "function pastResultsDeltaHtml(current,previous)",
    "function pastResultsBestByAthlete(rows,predicate)",
    "No saved prior result found.",
    "todayByAthlete=pastResultsBestByAthlete(rows,pastResultIsCurrentMeet)",
    "previousByAthlete=pastResultsBestByAthlete(rows,function(row){return !pastResultIsCurrentMeet(row);})",
    "function openPastResults()",
    "function refreshPastResults()",
    "Recall saved SMART Trak races for today's athletes.",
    "var shortcut=raceSummaryShortcutHtml(forceTools)+pastResultsShortcutHtml(forceTools);",
    "if(CL.type==='meet'&&!forceTools)return'';",
    ...
    "preserveRaceSummarySnapshot('partner-reset');",
    "preserveRaceSummarySnapshot('timer-reset');",
    "Race Summary",
    "saved result",
  ...
  [
    'id="tray-results-btn"',
    "var resultsBtn=document.getElementById('tray-results-btn');",
    "if(resultsBtn)resultsBtn.hidden=!meet;",
  ].forEach((text) => {
    if (mobile.includes(text)) throw new Error(`Mobile meet race summary should not use tray icon: ${text}`);
  });
...
function checkMeetHistorySportToolbarFilter() {
  ...
  const required = [
    ...
    "function filterVoidedSnapshotMeetRows(rows,voidedKeys)",
    "var recordKeys=Array.isArray(voidedKeys&&voidedKeys.recordIds)?voidedKeys.recordIds:(Array.isArray(voidedKeys)?voidedKeys:[]);",
    "if(recordId)return !blockedRecords[recordId];",
    "function snapshotMeetResultFingerprint(row)",
    "return item.sourceRecordId===sourceRecordId&&(!item.fingerprint||item.fingerprint===fingerprint);",
    "var snapshotRows=snapshot&&snapshot.ok&&snapshot.data? filterVoidedSnapshotMeetRows(snapshot.data.recentMeetResults||[],dashboard.data.voidedMeetResultKeys||[]):[];",
...
function checkMeetHistoryImportedResultCorrections() {
  ...
  [
    ...
    'id="historyEditSplits"',
    "<th>Actions</th>",
    "function openHistoryEdit(rowKeyValue)",
    "function historyEditGenderValue(value)",
    "function saveHistoryEdit()",
    "els.historyEditSplits.value=splitsInputFromText(historyEditRow.splitsText)",
    ...
    "splitsJson:splitsJsonFromInput(els.historyEditSplits.value)",
    "splitsText:splitsJsonFromInput(els.historyEditSplits.value)||historyEditRow.splitsText||''",
...
function checkMeetResultSplitDetails() {
  ...
  [
    ...
    "function splitsInputFromText(text)",
    "function splitsJsonFromInput(text)",
    "function splitChangeDisplay(current,previous)",
    "function splitDetailHtml(split,index,splits)",
    "function normalizedXcEventLabel(value)",
    "function historyEventFilterValue(row)",
    "events[historyEventFilterValue(row)]=true;",
    ...
    "function raceSheetXcRows()",
    "function raceSheetRowMatchesStatus(row,status)",
    ...
    "function quickExistingResultFor(input)",
    "function quickCorrectionPayload(existing,payload,row)",
    "var endpoint=existing?'/api/smart-trak/correction':'/api/smart-trak/meet-result';",
    "status:(existing?'Update - ':'Add - ')+athlete.name",
    "sportText(row)==='Cross Country'",
    "Exports cross country progression with athletes down the left and selected races across the top.",
    "var raceHeader=['Athlete','Grade'];",
    "var dateHeader=['',''];",
    "var columnHeader=['Athlete','Grade'];",
    "columnHeader.push('Final Time');",
    "columnHeader.push('Best Status');",
    "columnHeader.push('Race Change');",
    "cells.push('DNR','DNR','');",
  ...
  [
    ...
    "Distance + Gender",
    "Distance + Gender + Grade",
    "athletes down the left and selected races across the top",
    "race change from the athlete's previous selected race",
...
function checkMeetHistoryPostRaceSummary() {
  ...
  [
    ...
    "function postRaceEventKey(row)",
    "function postRaceResultKey(row)",
    "function postRaceBestRow(current,next)",
    "function postRaceSummaryRows(results)",
    "var rows=postRaceSummaryRows(results);",
    "var cells=[String(index+1),row.athleteName||'Unnamed',historyEventFilterValue(row)||'Race'];",
...
function checkMeetHistoryQuickEntry() {
  ...
  [
    ...
    "var endpoint=existing?'/api/smart-trak/correction':'/api/smart-trak/meet-result';",
    "fetch(endpoint,{method:'POST',headers:headers,body:JSON.stringify(requestBody)}",
    ...
    ".quickentrypanel{width:min(1400px,calc(100vw - 24px))}",
    ".quicktablewrap table{width:100%;min-width:0;table-layout:fixed}",
    ".quicktablewrap th:nth-child(7),.quicktablewrap td:nth-child(7){width:86px}",
...
function checkMobileIndividualLapButtonCues() {
  ...
  [
    ...
    "function runnerLapButtonStyle(r)",
    "function updateRunnerLapButton(btn,r)",
    "runnerLapButtonClass(r)+'\"'+runnerLapButtonStyle(r)+' id=\"lapb-'",
...
function checkAttendanceMobileSummary() {
  ...
  [
    ...
    "function attendanceSummaryDisplayKey(row)",
    "function compactLocalAttendanceDuplicates()",
    "if(key&&!byKey[key])byKey[key]=row;",
    "compactLocalAttendanceDuplicates();",
    "function attendanceSummaryDaysPossible(rows)",
    "return (rows||attendanceSummaryRows()).reduce(function(max,row)",
    "attendanceSummaryDaysPossible(rows)",
    ...
    "attendanceSummaryHeader('name','Name')",
    "attendanceSummaryHeader('rate','%')",
    "attendanceSummaryHeader('a','A')",
    '<span class="slbl">Range</span>',
    '<span class="slbl">Status</span>',
    "setAttendanceSummaryStatus(this.value)",
    '<option value="absent"',
    "No athletes match this status filter.",
    '<option value="custom"',
    ...
    "attendanceSummaryHeader('p','P')",
    "attendanceSummaryHeader('t','T')",
...
function checkMobileWorkflowOpeningFlow() {
  ...
  [
    ...
    'id="workflow-list"',
    ".workflow-row{width:100%;",
    "touch-action:pan-y",
    ".quick-row{width:100%;",
    ".lrow{display:flex;",
    ".picker-row{touch-action:pan-y}",
    ...
    "var WORKFLOW_TOUCH={x:0,y:0,moved:false};",
    "function workflowTapAttrs(action)",
    'ontouchstart="workflowTouchStart(event)"',
    'ontouchmove="workflowTouchMove(event)"',
    'ontouchcancel="workflowTouchCancel()"',
    "function workflowEventPoint(e)",
    "function workflowTouchStart(e)",
    "function workflowTouchMove(e)",
    "if(dx>4||dy>4)WORKFLOW_TOUCH.moved=true;",
    "function workflowTouchCancel()",
    "WORKFLOW_TOUCH.moved=true;",
    "function workflowPointerDown(e)",
    "function workflowPointerMove(e)",
    "function openWorkflowFromRow(e,id)",
    "if(WORKFLOW_TOUCH.moved){if(e)e.preventDefault();return;}",
    "workflowTapAttrs('openWorkflowFromRow(event,&quot;",
    ...
    "workflowTapAttrs('openGroupFromRow(event,",
    "function openGroupFromRow(e,id)",
    "if(WORKFLOW_TOUCH.moved){if(e)e.preventDefault();return;}",
    "oGroup(id);",
    "if(GV==='meets')",
    "['attendance','field','keep','equipment'].forEach(function(key){if(buttons[key])buttons[key].hidden=true;});",
    "function setCurrentGroupTitle()",
    "if(title&&CL)title.textContent=CL.type==='meet'?meetDisplayTitle(CL):CL.name;",
    ...
    "workflowTapAttrs('openTrainingModeFromRow(event,&quot;timer&quot;)",
    "workflowTapAttrs('openTrainingModeFromRow(event,&quot;speed&quot;)",
    "workflowTapAttrs('openTrainingModeFromRow(event,&quot;calendar&quot;)",
    "function openTrainingModeFromRow(e,mode)",
...
function checkMobilePowerTrakWorkflow() {
  ...
  [
    ...
    "var athlete=activeAthleteForRunner(runner)||findAthleteByName(name)||{};",
    "gender:runner&&runner.gender||runner&&runner.sex||athlete.gender||athlete.sex||''",
    "grade:runner&&runner.grade||runner&&runner.classYear||athlete.grade||athlete.classYear||''",
  ...
  [
    ...
    "function normalizePowerTrakBlocks(items)",
    "function normalizePowerTrakExercises(items, blockLabel)",
    ...
    "actualRir: result && result.actualRir",
    "prescribedReps:",
    "actualReps:",
    ...
    "lastPowerTrakSync",
    'cleanSetupText(payload.action).toLowerCase() === "cleanup-power-import"',
    'cleanSetupText(item.id).startsWith("power_import_")',
...
function checkPartnerTimingPhaseOne() {
  ...
  [
    ...
    "function crossCountryMeetEventLabels()",
    "function meetSaveEventLabels()",
    ...
    "id=\"m-xc-tools\"",
    "body.innerHTML=meetGroupSetupHtml(true)+groupSyncSummaryHtml(true);",
    "sortLabel.textContent=meet?(xc?'XC Tools':'Track Tools'):'Sort';",
    "sumLabel.textContent=meet?(xc?'Partner':'Relay'):'Sum';",
    "if(planBtn)planBtn.hidden=!!meet;",
    "if(CL&&CL.type==='meet'){if(isCrossCountryRaceMode())openXCTools();else openTrackTools();return;}",
    ...
    "function meetRaceSaveKey(eventName)",
    "function meetRaceAlreadySaved(eventName)",
    "has already synced. Do you want to sync again?",
    "markMeetRaceSaved(eventName);",
    "meetSyncedRaceKeys:Array.isArray(l.meetSyncedRaceKeys)?l.meetSyncedRaceKeys:[]",
    "sportMode:raceSportMode(l)",
    "sportMode:meta.sportMode",
    ...
    "Partner taps loaded. '+after+' shared tap",
    "CL.cfg.laps=1;",
    "partner-just-marked",
    "selectedStationIds",
    "Continue Race Clock",
...
function checkFieldPracticePhaseOne() {
  ...
  [
    ...
    "function fieldPracticeSpeedType(value)",
    "function fieldPracticeDistanceOptionHtml(values,selected)",
    "function fieldPracticeSpeedSetupLabel(defaults)",
    ...
    "flyZoneDistance",
    "speedSetupTouched",
    "FIELD_PRACTICE.speedSetupOpen=false;",
    "var setupOpen=!!FIELD_PRACTICE.speedSetupOpen||!p.speedSetupTouched;",
    "Metric '+ex(defaults.distance)+' '+ex(defaults.unit)",
    "fieldPracticeSpeedType(p&&p.focus||first.zone)",
    "Speed Metric is the timed distance used for MPH and velocity.",
    "<span>Surface</span>",
    "<span>Timing</span>",
    "<span>Speed Focus</span>",
    "<span>Start Type</span>",
    "field==='customSurface'",
    "row.timedDistance=defaults.distance",
    "row.distanceUnit=defaults.unit",
    "updateFieldPracticeSpeedSetup(&quot;zone&quot;,this.value)",
    "updateFieldPracticeSpeedSetup(&quot;flyZoneDistance&quot;,this.value)",
    "updateFieldPracticeSpeedSetup(&quot;distance&quot;,this.value)",
    "speed-hero",
    "speed-ath-open",
    "function startFieldPracticeSpeedToggle(event,key)",
    "function finishFieldPracticeSpeedToggle(event,key)",
    "function cancelFieldPracticeSpeedToggle()",
    "if(dx>10||dy>10)return;",
    "onpointerdown=\"startFieldPracticeSpeedToggle",
    "onpointerup=\"finishFieldPracticeSpeedToggle",
    "speed-ath-remove",
    "speed-add-rep",
    "speed-rep-collapsed",
    "function fieldPracticeOpenSpeedRepId(key,rows)",
    "function openFieldPracticeSpeedRep(key,id)",
    "FIELD_PRACTICE.openSpeedRepByAthlete[key]=next.id;",
    "String(row.id||'')!==String(openRepId||'')",
    ...
    "CL.trainingRaceManualOrder&&CL.trainingRaceManualOrderSession",
  ...
  [
    ...
    "\\s*(s|sec|secs|second|seconds|min|mins|minute|minutes)\\b",
