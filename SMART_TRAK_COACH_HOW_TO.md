...
Use **Rack Mode** when athletes are ready to complete a saved workout. Give the rack a name, choose one workout, and select up to six active athletes sharing that iPad. Power Trak uses the workout's event group and the current SMART Trak Groups list to place matching athletes first and label them **Recommended**. All active athletes remain available so coaches can handle multi-event athletes, substitutions, and group exceptions. If a new athlete is not yet on the official SMART Trak roster, use **Add Provisional Athlete**, enter their first name, last name, and graduation year, and optionally add gender and event group. Power Trak gives the athlete a permanent provisional ID, marks them **Needs Roster Review**, and makes them available immediately without adding them to official roster counts or unrelated team reports. The same control can add a late provisional athlete directly to an active rack. An athlete assigned to another active rack is unavailable, preventing the same athlete from recording on two racks at once. **Active Racks** shows each rack's iPad name, participating athletes, completed and prescribed reps, percent complete, and latest update time; an older update time is highlighted so the coach can check that rack's connection or progress. Use **Release Athlete** when someone changes racks; their completed reps remain in the original session, and they immediately become available for another rack. Releasing the final active athlete completes that rack. Everyone at a rack receives the same workout, but each athlete keeps an independent place in the exercise, round, and rep sequence. Tap an athlete's name before recording a rep, enter the actual load or result when needed, and tap **Complete Rep** once for each completed repetition. Load, height, distance, and time exercises require a valid actual result and unit before the rep can advance; the missing field is highlighted and focused. Completion-only exercises remain one tap. The button briefly reads **Saving Rep** and prevents another completion tap until that rep is stored or safely queued on the iPad. For fixed-load, percentage, and athlete-entered load exercises, that athlete's last entered load carries into the next rep and later rounds of the same exercise; changing it affects the new rep only. Use the minus and plus controls to change a carried load by 5 lb or 2.5 kg, or tap the value to type a different load. A coach can enable **Record RIR** on a load-bearing exercise and choose a target from `0` through `5+`. Before each rep is completed, the athlete must tap the actual reps-in-reserve value; Power Trak saves the actual and target RIR beside that rep's weight. Sessions displays both values, saved RIR can be corrected with **Edit**, and **Export Results** includes Target RIR and Actual RIR columns. Exercises without Record RIR continue to work exactly as before. Time, distance, and height fields remain independent for every attempt. Completing the final rep of the full workout marks that athlete complete, releases their rack assignment, and opens the next active athlete. When everyone finishes, Power Trak completes the rack automatically. After the final rep of an earlier exercise, that athlete's prescribed rest countdown begins and blocks only that athlete's next rep. Power Trak automatically opens the next ready athlete while the resting athlete's tab shows the remaining time; any athlete can still be selected manually. The timer survives saving or reopening the rack. Use **Skip Rest** when the coach wants that athlete to continue early. Use **Undo Last Rep** to correct the most recent tap and cancel its rest. If an athlete arrives after the rack starts, use **Add late athlete**; matching athletes are listed first, and the new athlete begins at the start of the workout without resetting anyone else. Use **Complete Rack** when the rack is done early. At the end, the rack receipt shows each athlete's recorded and prescribed reps, whether they completed or stopped early, and whether the final update has synced before the iPad returns to Rack Setup. When editing a saved measurable rep in **Sessions**, enter a nonnegative number and a unit. Power Trak leaves the saved rep unchanged when either is invalid and normalizes common pound and kilogram aliases to `lb` or `kg`.

Use **Sessions** to review active and completed rack workouts. Filter by status or workout, or search by rack or athlete. Use **Edit Session** to open a correction window for the rack/session name and workout date while preserving all athletes and reps. Invalid names or calendar dates remain unsaved, and a failed backend save leaves the original session unchanged. Open a session to see every athlete's recorded exercises, rounds, reps, and actual results. Use a rep's **Edit** control to open the saved-rep correction window and change its result or unit, or use its **Delete** control to open a confirmation that identifies the athlete, exercise, round, and rep before removing only that record. The correction is applied only after the server saves it successfully. **Export Results** downloads one CSV row per completed rep and honors the current status, workout, and search filters. An active session can be resumed from this view. Delete a rack session only when the entire saved rack workout is incorrect; Power Trak asks for confirmation before removing all of its athlete reps, and a delayed offline copy from a rack iPad cannot restore the deleted session.
