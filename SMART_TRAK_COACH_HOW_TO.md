...
Use **Athlete Progression** to compare one athlete across like-for-like testing conditions. Choose the season first, then the athlete and one speed metric. Speed Trak does not combine unlike metrics when calculating improvement or PBs. If the athlete has only one metric in that season, it is selected automatically; otherwise, choose the metric before results appear. Surface and timing method remain optional filters within that metric. **Current season** lists only active athletes who have speed results in the current season. Older season choices list all athletes who have results saved in that season, including graduated or inactive athletes. SMART Trak shows the comparable best, latest session best, change from the first matching session, total reps, and a session history with average, range, fly zone, start type, and chronological New PB indicators.
