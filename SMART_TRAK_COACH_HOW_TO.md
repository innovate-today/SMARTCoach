...
On the SMARTCoach app, tap a training group and choose **Speed Metrics** when the group is doing fly zones, acceleration zones, or runway timing. The app opens a compact speed screen and preloads the group athletes collapsed. Tap an athlete to open that athlete's reps, use the single **Start/Stop** button to time a rep, enter stride count, and add notes as needed. **Remove athlete from this session** only removes that athlete from the current speed session after confirmation; group membership is still managed in SMART Trak. Use **Results** to review the session leaderboard, including best reps sorted by velocity, stride length, stride frequency, time, name, or gender. Tap **Field** from the main SMARTCoach screen for field-event drill sessions and jump attempts.
