...
On cross country race screens, tap **XC Tools** for non-timing items such as Division, race athlete setup, Race Summary, Past Results, and save/status review. If the race opens without athletes, SMARTCoach opens XC Tools automatically and the race screen also shows **Open XC Tools**. Choose the division, choose the racing group, then tap **Load** to bring those athletes into the race. After Load, SMARTCoach closes XC Tools and returns to the timing screen. Make last-minute scratches with the red `x` on the race screen or add an athlete with **Add**. The live race screen stays focused on runner names, times, Start/Lap/Stop, athlete info, and removing last-minute scratches. Track meet screens keep **Field** and **Relay** in the tray instead.
