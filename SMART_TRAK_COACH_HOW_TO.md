...
Use **Rack iPad Login** to open the dedicated athlete-entry version of Power Trak. Enter the same SMARTCoach account key and shared access code on each rack iPad, give each device a recognizable name such as `Rack 2 iPad`, and add the page to the iPad Home Screen when desired. The installed view remembers that device and opens only Rack Mode; workout creation, imports, history, analytics, and dashboard navigation remain outside the athlete-facing view. This foundation still requires an internet connection for saves; offline rep queuing is a later implementation phase.
