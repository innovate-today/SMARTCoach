...
Use **Speed Trak → Sessions** as the completed-history view for speed testing. It shows each testing date and setup with its athletes, reps, fastest mark, velocity, and MPH. Dashboard **Training Load → Completed Workouts** is reserved for distance-training workouts, so Speed Metrics sessions do not add distance-training workout counts or volume there.
