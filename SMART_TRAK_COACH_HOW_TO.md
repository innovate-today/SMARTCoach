...
4. Choose which details should appear: **Latest Meet**, **Season Results**, **Meet Archive**, **Athlete Summary**, **Event Summary**, **Division Summary**, **PB / SB Highlights**, **PB / SB Badges**, and **Team Summary**. Use the **Up** and **Down** controls to place the board sections in the order you want, or use **Defaults** to restore the standard section order.
...
7. After opening the public board, use the board filters to create special views. For example, choose **Girls** and tap **Share View** to copy a girls-only board link, then choose **Boys** and tap **Share View** again for a boys-only link. Those filtered links preserve the selected meet, event, gender, search text, and display mode, so they can be shared or displayed wherever the coach wants them.

The public Results Board is read-only. It includes the latest meet for the selected sport/year, team totals, PB/SB totals, top girls result, and top boys result. If girls or boys race more than one distance in the latest meet, the board splits that gender into separate top-result cards for each race distance. The race distance and time stay under the athlete's name. Result rows show the athlete's best posted time for that event in the **Best** column, with a **New PB** indicator when the result matches or improves that best. It also includes PB/SB Highlights, a Division Summary for Boys/Girls totals, a Meet Archive with meet-by-meet result counts, an Athlete Summary with season totals by athlete, an Event Summary with event leaders, and a Season Results table that shows each athlete's best saved result by event. Viewers can filter by meet, event, gender, and search text, and they can tap any table column header to sort that table ascending or descending without seeing coach tools or edit buttons.
...
Speed Trak pulls from saved Speed Metrics reps. It can show common fly and start metrics such as **10m Fly**, **30m Fly**, **Max Velocity**, **30m Start**, **60m Start**, **90m Start**, **150m Start**, and **250m Start**. Coaches can filter by metric, surface, timing method, speed focus, start type, gender, year, athlete search, and how many athletes to rank. Surface choices include track, turf, grass, and indoors. Choose **Other** and enter the surface name when the needed surface is not listed. Use the setup filters when results should only be compared under like-for-like testing conditions, such as FAT rolling starts on the same surface.

Use the **Leaderboard** tab to compare each athlete's single best qualifying mark for the selected speed metric and filters. Slower reps remain available in **Sessions** and **Athlete Progression**, but do not create duplicate leaderboard rows. Use **Sessions** to review complete testing days. Filter sessions by test, surface, timing method, speed focus, start type, or year. Each session row shows the date, test, setup, surface, timing method, speed focus, athlete count, total rep count, and fastest mark. Click **Open** to inspect every saved athlete rep from that session. Rep numbers restart at Rep 1 for each athlete. Use **Edit** or **Delete** beside a rep to correct that specific saved attempt.

Use **Athlete Progression** to compare one athlete across like-for-like testing conditions. Open **Years** and check one or more workout years, then choose the athlete and one speed metric. Each year comes from the workout date, so every test dated in 2026 appears under 2026, including offseason work. The newest workout year is selected initially. Choosing multiple years combines their matching sessions into one chronological progression view. The athlete list includes athletes with results in any selected year. Speed Trak does not combine unlike metrics when calculating improvement or PBs. If the athlete has only one metric in the selected years, it is selected automatically; otherwise, choose the metric before results appear. Surface and timing method remain optional filters within that metric. The **Best Time Trend** chart follows those same filters and plots each session's best mark; lower time values are faster, and green points identify a new chronological PB. Hover over or tap a chart point to see its exact date, best time, rep count, and PB status and highlight the matching session row. Hover over, focus, or tap a **Best** value in the session table to open that same bubble on its matching chart dot and highlight the row. On a phone, swipe the compact session table sideways to review the remaining setup columns without shrinking the chart or its controls. SMART Trak also shows the comparable best, latest session best, change from the first matching session, total reps, and a session history with average, range, fly zone, start type, and chronological New PB indicators.

To import speed marks, open the **Import Data** tab on the Speed Trak page. Download the template, upload a CSV/TSV file, or paste spreadsheet rows. Required information is athlete name, mark/time, and either a date in the row or the default date above the paste box. Metric can be included in the spreadsheet or supplied by the default metric. Event labels may use meters or yards, such as **10m Fly**, **10yd Fly**, or **40yd Dash**; SMART Trak preserves the stated unit and converts yards internally for velocity and MPH calculations. Speed imports can also carry the test description, surface, timing method, speed focus/category, start type, timed distance and unit, gender, graduation year, and season year. SMART Trak converts graduation year into the athlete's grade for the workout date. Repeated rows for an athlete remain separate reps. Rows with the same date and test setup are saved together as one testing session; changing the event, surface, timing method, focus, start type, fly zone, or timed distance creates a separate session. The preview shows the athlete, event, mark, grade, test setup, and how many reps and sessions are ready. If the same spreadsheet is previewed again, exact occurrences already stored in Speed Trak are labeled **Already saved** and skipped. Additional repeated rows beyond the number already stored remain ready, so legitimate equal-time reps are preserved. If rows need fixes, choose **Show Rejected Rows** to see every rejected row with its original spreadsheet row number and the reason it will not be saved. Use **Show Preview** to return to the regular preview. If the athlete name matches the saved roster, SMART Trak links the mark to that roster athlete to help avoid duplicate athlete entries. Preview the rows before clicking **Save Speed Data**. Large historical files may take a moment to save; remain on the page until SMART Trak confirms the rep and session counts saved.

You may paste a full source export without deleting its extra columns. Keep the header row. Speed Trak reads the recognized headers by name, including underscore versions such as **Timing_Method**, **Distance_Unit**, and **Graduation_Year**, while safely ignoring **Event_ID**, **Group**, **Result_Unit**, **Better**, **Class**, **Yearly_Attempts**, **Total_Attempts**, and **PR**. Extra columns do not shift the recognized values.

In the import preview, **Active** and **Inactive** mean the athlete matched a saved roster record. An unmatched athlete whose graduation year has passed is labeled **Historical** so graduated athletes are not presented as roster errors. **Unmatched** remains for current or future athletes whose names do not match the roster and may need review.

Owners and admins can use **Manage Imported Data** to remove a mistaken Speed Trak spreadsheet import in one protected batch. Add the import date, workout date, or both to narrow the search, then choose **Preview Cleanup**. Review the session, rep, athlete, workout-date, and import-date counts before continuing. Deletion stays disabled until the exact confirmation phrase from that preview is entered. This cleanup includes only spreadsheet-imported Speed Trak sessions; results saved from the SMARTCoach app or entered manually are not removed.
...
Use **Export Data** to download every saved Speed Trak rep as a CSV file. The export includes slower reps as well as each athlete's leaderboard mark, and preserves dates, test setup, distance and unit, athlete details, notes, velocity, and MPH. The exported columns can be used for offline review or reimported later after removing the calculated Velocity and MPH columns if desired.
...
Use **Share Board** to create a read-only Speed Trak Board link. Choose the public challenge layers you want to show, such as top velocity, fastest time, and game score. You can also adjust public badge values for **Velocity Club**, **Quick Turnover**, and **Big Mover** before copying the link. The shared board can be filtered by distance/metric, surface, timing method, speed focus, start type, gender, and year. Current coach filters are included when the link is created, and viewers can adjust the same filters on the public board. Setup filters are applied before each athlete's best qualifying result is selected, keeping comparisons like FAT rolling starts on grass separate from unlike tests. Reps remain visible as supporting detail, but they are not used as a Rep Leader award.

Use **Speed Trak → Sessions** as the completed-history view for speed testing. It shows each testing date and setup with its athletes, reps, fastest mark, velocity, and MPH. The year filter uses the workout date's calendar year, so every workout dated in 2026 remains under 2026 and the coach can recognize offseason work from its date. Choose **Open** to expand the details directly below that session; choose **Close** to collapse it, or open another session to move the details there. Choose **Edit Session** when shared setup information was entered incorrectly. One save updates the date, test, timed distance and unit, fly-zone distance, surface, timing method, speed focus, or start type for every athlete and rep in that session. Athlete names, individual marks, grades, and notes are not changed. Choose **Delete Session** only when the entire test is invalid; after confirmation, every athlete rep in that session is permanently removed. Dashboard **Training Load → Completed Workouts** is reserved for distance-training workouts, so Speed Metrics sessions do not add distance-training workout counts or volume there.
...
9. Optional: for **Full Access** assistants, add an assistant coach email, then click **Create Invite** or **Copy Invite** to copy one message with the assistant's personal code, phone app link, and one private SMART Trak invite link. Copying an invite creates/resets that assistant's personal code, so send the newest copied message.
...
When a training group is using **Training Calendar** from SMART Trak, refreshing the SMARTCoach app moves the group to the current upcoming Training Calendar workout for that group. This keeps the phone app from holding an older selected Training Calendar workout after the Training Calendar has moved to a new day.
...
When importing a roster, click **Import Athletes**, then either upload a CSV file or paste spreadsheet rows into the roster box. The first row must contain column names. Use **Download Template** if you want a clean starting file. SMART Trak can fill the same roster fields shown when you edit an athlete: first name, last name, gender, class year, athlete email, group, active status, parent name, parent email, parent phone, second parent name, second parent email, second parent phone, and notes. Gender can be listed as Boy, Girl, B, G, M/F, Boy/Girl, or similar. Parent names can be in columns such as Parent Name, Guardian 1, Parent 1, Mother, Guardian 2, Parent 2, or Father, or split into first name and last name columns. If you include a group, use the same group name already saved in SMART Trak. After the import finishes, open one athlete and spot-check the gender, class year, group, and parent contact details.
...
Inactive athletes are removed from the Attendance sheet after the app loads the current roster. If the roster has not loaded yet, the app asks the coach to refresh instead of showing old group members. Old attendance marks for athletes who later become inactive can still appear on the Attendance review page so you can audit past records.
...
5. Tap **Refresh Data** if you need to reload the equipment records for the selected group.
6. For each active athlete, mark each item **Issued**, **Returned**, **Lost / Damaged**, **Not Required**, or **Not Issued**.
7. Enter sizes, numbers / IDs, or notes as needed.
8. Tap **Save** on one athlete card or **Save Equipment** to save the whole visible group to SMART Trak.
...
For Easy Run workouts, leave **Target** blank to use the standard easy target from each athlete's current fitness. If no current fitness is set, SMARTCoach treats the easy run as conversational. In the SMARTCoach app, the workout card type controls the target first: Easy Run uses the easy/recovery pace target, Quality Session uses the set effort such as Threshold or Interval, Speed Metrics uses speed work instructions, Race uses race-day information, and Rest / Day Off has no pace target. Workout notes such as `6 mi easy` or relaxed strides add detail but should not turn an easy day into a rep-distance target.
...
5. For runway checks, fly zones, max velocity, or sprint metrics, choose the zone, timed distance, and unit, then load the athletes. Time each rep or enter a manual time, and SMART Trak calculates velocity and MPH.
...
On the SMARTCoach app, choose **Speed Trak**, then choose the group when the group is doing fly zones, acceleration zones, max velocity, or runway timing. The app opens a compact speed screen and preloads the group athletes collapsed. In **Speed Setup**, choose **Fly** or **Start**. For Fly work, choose the Fly Zone distance and the Speed Metric distance. For Start work, choose the Speed Metric distance. Also confirm the session surface, timing method, speed focus, and start type; those choices apply to every rep in the session and keep Sessions, Leaderboard, Athlete Progression, filters, and exports comparable. Choose **Other** to enter a surface not listed. The Speed Metric distance is the timed distance used to calculate MPH and velocity. Meter and yard labels stay as selected when the session appears in Speed Trak history; yards are converted only for velocity and MPH calculations. Tap **Done** to accept the displayed setup and collapse the panel, including when the default 30m values are already correct. After setup, those choices stay visible as pills at the top of the screen; tap a pill to reopen setup. Tap the **i** beside an athlete to see that athlete's saved PB for the selected test distance and type, such as 30m Fly; other speed-test distances stay out of the small popup. Tap an athlete to open that athlete's reps, use the single **Start/Stop** button to time a rep or enter a manual time, and add notes as needed. Once time is entered, the rep shows velocity and MPH from the timed distance and time. After a rep has enough information, the app confirms the rep is saved and tells the coach to move to the next runner or add the next rep. Use the compact **+ Rep** and **Remove** controls when an athlete needs another rep or an incorrect rep needs to be removed; removing a rep asks for confirmation first. Adding a rep automatically collapses the previous rep and opens the newest one. Older reps remain as compact summary rows and can be tapped when they need to be reviewed or edited, while only one full rep stays open for that athlete. Drag the handle beside a name to reorder the session list without opening details. **Remove athlete from this session** only removes that athlete from the current speed session after confirmation; group membership is still managed in SMART Trak. The session leaderboard appears at the bottom automatically and shows best reps sorted by velocity, time, name, or gender. Tap **Save to SMART Trak** when the session is ready to sync to Speed Trak. Tap **Field** from the main SMARTCoach screen for field-event drill sessions and jump attempts.

## Power Trak

On each dedicated rack iPad, tap **Device Check** before athletes begin. Confirm that internet connection, offline rack storage, installed rack support, SMART Trak connection, protected rack save path, and pending rack updates all show **Ready**. The save-path check verifies authentication, account storage, and the rack mutation lock without creating or changing any Power Trak data. A passing result is shown in the rack header for 12 hours on that iPad; run the check again when it expires or says it needs attention. If a rack is started without a current passing check, Power Trak warns first. Cancel and run the check whenever possible; use **Start Anyway** only after the coach has verified that iPad another way. The check does not reserve an athlete or start a rack.
...
On the Dashboard, open **Power Trak**. Use **Workouts** to create reusable rack workouts. Give the workout a name and event group, then organize exercises into A, B, C, or later blocks. Set the number of rounds and reps for each exercise, an optional coaching cue, and optional rest seconds after that exercise. For **Execution**, choose Both together, Each side, Alternating, Left only, or Right only. Each side and Alternating prescribe the entered rep count per side; Rack Mode records actual left and right reps separately and includes both counts in Sessions and exports. Existing workouts default to Both together. The Exercise and 1RM Exercise fields suggest names from the account's Exercise Library; choose a suggestion for consistent analytics or type a new exercise when needed. A selected exercise can be typed over, **Clear** keeps the row but empties its exercise fields, and **Remove** deletes the row from that workout. Use **Manage Exercises** to add, rename, or remove the suggestions available to coaches. Library changes do not rewrite exercises already saved inside workouts. Choose whether athletes use a fixed load, a percentage of their saved one-rep max, an athlete-entered load, completion only, height, distance, or time. Percentage workouts automatically use the athlete's best matching one-rep max from Testing History or saved rack/import results. Exercise names such as `Barbell Back Squat` and `Back Squat` are matched, while pound and kilogram records stay separate. The prescribed load rounds to 5 lb or 2.5 kg. Rack Mode shows the coaching cue and prescribed rest with the current exercise. Save the workout when every exercise is named. Duplicate an existing workout when a new workout only needs a few changes. **Archive** an old template to keep it in the Workout Library and preserve its completed sessions while removing it from Rack Mode choices; select it later and use **Restore** to make it assignable again.

Use **Rack iPad Login** to open the dedicated athlete-entry version of Power Trak. Enter the same SMARTCoach account key and shared access code on each rack iPad, give each device a recognizable name such as `Rack 2 iPad`, and add the page to the iPad Home Screen when desired. The installed view remembers that device and opens only Rack Mode; workout creation, imports, history, analytics, and dashboard navigation remain outside the athlete-facing view. The device name supplies the first rack name, so `Rack 2 iPad` starts with `Rack 2`; an edited rack name and the last selected active workout are remembered separately on that iPad. If the page reloads while that iPad owns an active rack, Power Trak automatically reopens the same rack at each athlete's saved position instead of returning to setup. The same recovery works from the latest saved iPad copy during a brief outage. Checking an athlete reserves them immediately for that iPad, and the other rack iPads remove that athlete from selection within a few seconds. Unchecking releases the athlete immediately. Reservations renew while the setup screen remains open and expire after five minutes if an iPad closes or loses power before starting its rack. Starting the rack converts each reservation into the normal active-rack assignment. Sign-in, athlete reservations, and starting a new rack require an internet connection. After an active rack and its workout have loaded, a brief outage does not stop rep entry: Power Trak keeps the latest complete rack state on that iPad, displays the number of updates waiting to sync, and retries automatically every ten seconds and when the connection returns. The same queue protects a rep when another rack or coach save is briefly using the account; temporary backend contention retries automatically instead of discarding the athlete's tap. If updates remain pending after the connection returns, tap **Retry Sync**. If a coach changed, completed, or deleted that rack while the iPad was disconnected, the older queued copy is cleared and the iPad reloads the coach's authoritative state instead of retrying forever. Power Trak warns before leaving the page or changing the rack login while pending updates belong to that device/account queue. The in-app login-change warning shows the number of queued updates and explains that they belong to the current device and account before allowing the session to be cleared. Keep that iPad on the same rack login until its status reads **All rack data synced**, and do not clear its browser data while updates are pending.

Power Trak's automated acceptance coverage verifies the critical multi-iPad ownership sequence: one rack can reserve an athlete, a second rack is rejected while that reservation exists, a different device cannot release the owner's reservation, abandoned reservations expire after five minutes, releasing from the owning rack makes the athlete available, the reservation owner can start the rack, a different device cannot start from another iPad's reservation, and an athlete already active in a rack remains unavailable elsewhere. Offline queue coverage verifies that repeated saves for one rack retain only its newest complete snapshot, separate racks remain independently queued, saved snapshots cannot be mutated afterward, and successful synchronization removes only the matching rack. Reconnect coverage also verifies that a failed rack remains queued without preventing later healthy racks from syncing, that the retained snapshot can succeed on a later retry, and that an older offline snapshot cannot reopen, recreate, or overwrite a rack the coach already changed. Server-managed rack revisions make this ordering independent of the clock settings on individual iPads. A live practice test with the school's actual iPads is still required to confirm local Wi-Fi, Home Screen installation, and reconnect behavior in the facility.

Use **Rack Mode** when athletes are ready to complete a saved workout. Give the rack a name, choose one workout, and name its two sides using the weight room's terminology (for example, Inside and Outside). Choose a **Starting block** for each side before starting the rack. All athletes complete every block once in rotation: with A, B, and C, a side starting at B works B, C, then A. Assign up to four athletes per side, eight total, on one iPad. Both sides share the workout template, but each athlete keeps an independent block, exercise, and set position; the sides can work on different blocks at the same time. Power Trak uses the workout's event group and the current SMART Trak Groups list to place matching athletes first and label them **Recommended**. All active athletes remain available so coaches can handle multi-event athletes, substitutions, and group exceptions. If a new athlete is not yet on the official SMART Trak roster, use **Add Provisional Athlete**, enter their first name, last name, and graduation year, and optionally add gender and event group. Power Trak gives the athlete a permanent provisional ID, marks them **Needs Roster Review**, and makes them available immediately without adding them to official roster counts or unrelated team reports. The same control can add a late provisional athlete directly to an active rack. An athlete assigned to another active rack is unavailable, preventing the same athlete from recording on two racks at once. **Active Racks** shows each rack's iPad name, participating athletes, completed and prescribed reps, percent complete, and latest update time; an older update time is highlighted so the coach can check that rack's connection or progress. Use **Move to [side]** with confirmation when an athlete switches sides mid-workout; saved sets and workout position stay with that athlete, and each completed set records the side where it happened. Use **Release Athlete** when someone changes racks; their completed sets remain in the original session, and they immediately become available for another rack. Releasing the final active athlete completes that rack.

Everyone at a rack receives the same workout, but each athlete keeps an independent place in the exercise and set sequence. Tap an athlete's name, enter the **Actual reps** and weight or result, then tap **Complete Set** once. Actual reps may be lower or higher than the prescription, so an athlete prescribed 8 reps can record 20 without changing the workout. Load, height, distance, and time exercises require a valid result and unit; completion-only exercises require only actual reps. The button briefly reads **Saving Set** and prevents a duplicate tap until the set is stored or safely queued. For load exercises, the athlete's last entered weight carries into later sets of that exercise. Use the minus and plus controls to adjust it by 5 lb or 2.5 kg, or type a different weight. A coach can enable **Use RIR target** and choose `0` through `5+` as workout guidance; athletes do not record an RIR value. Saved workload is calculated from actual reps multiplied by actual weight.

After every **Complete Set** tap, Power Trak opens the next ready athlete on that rack while preserving each athlete's independent exercise and set position. Resting and finished athletes are skipped; when only one athlete is ready, the screen stays with that athlete. Completing the final assigned set marks that athlete complete and releases their rack assignment. When everyone finishes, Power Trak completes the rack automatically. After a set, that athlete's prescribed rest countdown blocks only that athlete's next set. Use **Skip Rest** to continue early or **Undo Last Set** to remove the latest set and cancel its rest. If an athlete arrives late, use **Add late athlete**; the athlete begins at the start without resetting anyone else. Use **Complete Rack** when the rack is done early. The final receipt shows actual and prescribed reps, completion status, and sync status. In **Sessions**, a saved set's actual reps, result, and unit can be corrected or the set can be deleted. Existing historical per-rep records remain readable and count as one actual rep each.

Use **Sessions** to review active and completed rack workouts. Filter by status or workout, or search by rack, athlete, or exercise. Use **Edit Session** to correct the rack/session name or workout date while preserving all athletes and sets. Invalid names or calendar dates remain unsaved, and a failed backend save leaves the original session unchanged. Open a session to see every athlete's recorded exercises, sets, actual reps, and weight or result. Use a set's **Edit** control to change its actual reps, result, or unit, or use **Delete** to confirm removal of only that saved set. The correction is applied only after the server saves it successfully. **Export Results** downloads one CSV row per completed set, including prescribed reps and actual reps, and honors the current status, workout, and search filters. An active session can be resumed from this view. Delete a rack session only when the entire saved rack workout is incorrect; Power Trak asks for confirmation before removing all athlete results, and a delayed offline copy from a rack iPad cannot restore the deleted session.
...
Use **Import Data** when another strength system provides a CSV or tab-separated report. Choose the default result unit, then upload the file or paste the full report including its header row. Click **Preview Data** before saving. The preview reports recognized rows, generated sessions, athlete count, rows already saved, and rejected rows. Power Trak uses athlete name, completed or assigned date, exercise, set results and reps, completion status, highest max, volume load, opt-out status, and group. Source IDs, timestamps, checks, Max Allowed, and helper columns stay out of coaching views. Imported athletes are matched to the SMART Trak roster by name when possible. Reimporting the same rows updates the same imported sessions instead of creating duplicates. Imported report history does not create reusable Workout Library templates or appear as a Rack Mode workout choice. Use **Manage Imported Data** to preview and remove imported history after a wrong file or unit is used. The in-app cleanup confirmation shows affected session, athlete, and rep counts, keeps the required phrase visible while it is entered, and requires an exact match; manually completed racks, reusable workouts, and legacy Testing History stay saved.

Use **History** to review saved legacy testing sessions. The page can filter by group, metric, saved test, athlete, grade, gender, note, or test name. Use **Download CSV** to export the current filtered view. Use **Edit Mark** to correct one saved athlete name, grade, gender, mark, or note inside a test. Choose a saved test and use **Delete Test** only when the entire testing session was saved by mistake. The in-app confirmation identifies the selected test and explains that every athlete mark in that test will be removed while rack workouts, rack sessions, and other saved tests remain.
...
If a result was synced as a placeholder such as `runner 9` or `runner 10` because the athlete name was missed at the meet, go to **Dashboard > Recent Meet Results**, click **Edit**, choose the correct **Athlete**, enter a reason, and click **Save Correction**. After SMART Trak refreshes, the result will move to that athlete and the generated placeholder will not stay in active roster/setup lists.
...
The SMARTCoach app opens by asking what the coach is doing first, then which group to use. Choose **Training Trak**, **Race Trak**, **Speed Trak**, **Power Trak**, **Multi Athlete Timer**, or **Training Calendar**, then choose the group. The group list still uses the same SMART Trak roster source; the selected workflow controls what opens after the group is tapped.
...
In the SMARTCoach app settings, **Training/Race Order** controls how athletes appear when opening training groups, race groups, and Speed Trak sessions. The default is **Sport, speed, name**. SMARTCoach uses the saved **Current Fitness Setup** marks first, keeps athletes with the matching sport ahead of other sport marks, ranks faster marks first, then uses name A-Z when needed. Workout target ranges can still display on the timing screen, but they do not move athletes after the group opens. Saved times from the workout you are timing also do not reshuffle the group during practice. If a coach drags runners into a custom order during the session, that manual order stays in place for that open session. Attendance keeps its attendance-focused order and filters.

How to use it:

1. Open the SMARTCoach app.
2. Choose **Training Trak**.
3. Choose the group.
4. Choose the training mode when prompted, such as **Multi Athlete Timer**, **Speed Trak**, or **Training Calendar**.
...
When adding a Race day from SMART Trak Training Calendar, create the race without choosing a training group. Cross country race rosters are chosen later in the SMARTCoach app so each division can load the right group and make only small race-day scratches or additions.

On cross country race screens, tap **XC Tools** for non-timing items such as Division, race athlete setup, Race Summary, Past Results, and save/status review. If the race opens without athletes, SMARTCoach opens XC Tools automatically and the race screen also shows **Open XC Tools**. Choose the division, choose the racing group, then tap **Load** to bring those athletes into the race. After Load, SMARTCoach closes XC Tools and returns to the timing screen. Make last-minute scratches with the red `x` on the race screen or add an athlete with **Add**. The live race screen stays focused on runner names, times, Start/Lap/Stop, athlete info, and removing last-minute scratches.

On track race screens, tap **Track Tools** for **Current Meet Results** and **Past Results**. Current Meet Results loads saved SMART Trak results from the open meet and groups them by event, such as 100m, 200m, or 400m, so event results are not blended together. **Relay** stays in the tray as the direct relay timing control.
...
4. Tap **+ Add Field Event** and choose the field event, such as Long Jump.
...
- **Target on Detail Line:** show or hide target ranges. Timed quality sessions, such as `12 x 45 sec @ Threshold`, show the pace target for the timed work rep instead of changing the workout into a distance target.
...
If the app says **SMART Trak is busy. Please retry in a few seconds.**, wait a moment and tap the sync button again.
...
To identify the workout an event group should use today, open that saved workout and click **Assign Today**. Only one workout per event group can hold today's assignment; assigning another replaces the earlier choice for that group. Rack iPads place today's assigned workouts first and label them **Today**. When only one workout is assigned today, a rack iPad selects it automatically unless an active rack is being restored. Click **Clear Today** when the assignment should be removed. The label expires automatically on the next calendar day and does not alter completed sessions.

The Rack Setup preview identifies whether the selected workout is today's assignment. When at least one assignment exists and a rack tries to start a different workout, Power Trak lists today's assignments and asks for confirmation. Use **Start Anyway** only for an intentional exception; cancel and choose the assigned workout when the wrong workout was selected.

The coach's **Active Racks** list provides separate **Open** and **End Rack** controls. Use **Open** to enter a rack and review or assist with its live athlete workflow. Use **End Rack** only for an abandoned or incorrectly started rack; Power Trak requires confirmation, preserves every recorded rep, releases unfinished athletes, and closes the rack on its connected iPad. The athlete rack screen does not show this overview-level ending control.
