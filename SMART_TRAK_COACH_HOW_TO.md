...
Use **Rack iPad Login** to open the dedicated athlete-entry version of Power Trak. Enter the same SMARTCoach account key and shared access code on each rack iPad, give each device a recognizable name such as `Rack 2 iPad`, and add the page to the iPad Home Screen when desired. The installed view remembers that device and opens only Rack Mode; workout creation, imports, history, analytics, and dashboard navigation remain outside the athlete-facing view. Checking an athlete reserves them immediately for that iPad, and the other rack iPads remove that athlete from selection within a few seconds. Unchecking releases the athlete immediately. Reservations renew while the setup screen remains open and expire after five minutes if an iPad closes or loses power before starting its rack. Starting the rack converts each reservation into the normal active-rack assignment. Sign-in, athlete reservations, and starting a new rack require an internet connection. After an active rack and its workout have loaded, a brief outage does not stop rep entry: Power Trak keeps the latest complete rack state on that iPad, displays the number of updates waiting to sync, and retries automatically every ten seconds and when the connection returns. If updates remain pending after the connection returns, tap **Retry Sync**. Power Trak warns before leaving the page or changing the rack login while reps remain unsynced. Keep that iPad on the same rack login until its status reads **All rack data synced**, and do not clear its browser data while updates are pending.
