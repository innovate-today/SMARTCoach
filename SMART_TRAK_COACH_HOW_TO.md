...
Use **Speed Trak → Sessions** as the completed-history view for speed testing. It shows each testing date and setup with its athletes, reps, fastest mark, velocity, and MPH. The year filter uses the saved season year rather than the workout date's calendar year, so offseason tests remain attached to the intended track season. Choose **Open** to expand the details directly below that session; choose **Close** to collapse it, or open another session to move the details there. Choose **Edit Session** when shared setup information was entered incorrectly. One save updates the date, test, timed distance and unit, fly-zone distance, surface, timing method, speed focus, or start type for every athlete and rep in that session. Athlete names, individual marks, grades, and notes are not changed. Choose **Delete Session** only when the entire test is invalid; after confirmation, every athlete rep in that session is permanently removed. Dashboard **Training Load → Completed Workouts** is reserved for distance-training workouts, so Speed Metrics sessions do not add distance-training workout counts or volume there.
