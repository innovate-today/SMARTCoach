...
- Future Idea Trak phase 2: add a coach-facing idea board after beta feedback volume is understood. Phase 2 should let approved ideas appear publicly inside SMART Trak, allow coaches to upvote useful ideas, and let support mark ideas as New, Reviewing, Planned, Built, or Not Now. Phase 1 stays private: the desktop Feedback button saves Idea Trak submissions for internal review without adding a public board.
- Future MileSplit integration discovery: approach MileSplit/FloSports about an approved coach-authorized results feed into SMART Trak. Ask whether they offer a partner API, webhook, export feed, or structured result file for team-specific results; what authentication model they require; whether data can be limited to one school/team; whether historical seasons can be pulled; how corrections/updated results are communicated; what rate limits, costs, partner terms, attribution/link-back rules, and data-use limits apply; and whether live, race-by-race, final, or official-only results are available. Required fields for SMART Trak should include stable meet/event/result/team/athlete IDs, meet name/date, sport, season/year, event/race, gender/division, athlete name, MileSplit athlete ID, school/team, grade/class year, mark/result, place, wind, splits, relay legs, official status, source URL, and last-updated timestamp. SMARTCoach should be prepared to provide app name, domain/callback URL, technical contact, sample account/team, desired schools/teams, whether live or finalized results are needed, how imported results appear in private coach dashboards, and assurance that full meet databases will not be republished publicly. Preferred wording for the ask: "Do you have a partner API or approved export method that allows a coach-authorized app to pull a team's official MileSplit results into the coach's private team dashboard?"
- Partner Timing phase 1 is underway: add a shared cross country race-timing workflow so multiple coaches can record one race together from different course points. One coach creates a race session with split stations such as Start, 1600m, 3200m, 4800m, and Finish; each assistant joins the same race session, chooses a station, records only that station's taps, and syncs back to one combined race in SMART Trak. Phase 1 stores shared station taps at the account level, uses the Start timestamp plus station tap timestamps to calculate elapsed splits after sync, and flags missing or duplicate splits in review before official meet-result save decisions. Live real-time coordination can come later after the offline shared-race merge is stable.
- Future Partner Timing Video Assist: keep normal live taps, hooks, ordering, athlete pills, and the shared clock as the primary workflow. Add optional on-page checkpoint video with a synchronized whole-second race clock and movable timing line so coaches can recover missed splits from unexpected packs. V1 should use temporary local, no-audio recordings and let a paused video frame assign its historical race time through the existing athlete lap pills. Store timing metadata instead of burning the clock into video. Do not include AI, bib recognition, automatic crossing detection, hundredths, or automatic cloud uploads. Delete clips after confirmation and expire unreviewed local footage after about 24 hours; consider explicit cloud **Save to Race** only in a later phase after privacy, storage, and upload requirements are validated.
