...
- Near-term Partner Timing plan: add a shared cross country race-timing workflow so multiple coaches can record one race together from different course points. One coach creates a race session with split stations such as Start, 1600m, 3200m, 4800m, and Finish; each assistant joins the same race session, chooses a station, records only that station's taps, and syncs back to one combined race in SMART Trak. Phase 1 should work offline and merge by shared race session, athlete, and split point after sync, with desktop warnings for missing or duplicate splits. Live real-time coordination can come later after the offline shared-race merge is stable.
