...
function isTrainingSystemNoteLine(line) {
  ...
  if (/^(Workout|Planned target|Planned effort|Planned volume|Completed volume|Actual|Difference|Current fitness|Workout Location|Athlete note|Athlete notes|Weather|Correction Date|Correction Reason|SMARTCoach Status|Strava activity id|Matched calendar workout|Matched group|Strava activity|Strava start|Strava type|Strava distance|Strava moving time|Strava API laps exposed|Strava API standard splits exposed|Strava Lap|Strava Mile|Strava streams note):/i.test(text)) return true;
