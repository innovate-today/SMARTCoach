...
const { getGhlContext, coachCodeAllowed, coachSessionFromRequest, subscriptionAccessAllowed, subscriptionBlockedMessage } = require("../../lib/ghl-account");
...
async function athleteCalendar(req, res, context) {
  ...
  const visible = days.filter((day) => athleteCanSeeDay(athlete, groups, day)).map(publicDay);
  res.status(200).json({
    success: true,
    athlete: publicAthlete(athlete),
    days: visible,
...
function normalizeContact(contact) {
  const customFields = contact && (contact.customFields || contact.customField || contact.custom_fields || []);
  ...
  return {
    ...
    smartcoachAthleteId,
    smartcoachActive: activeValue(contactFieldValue(customFields, SMARTCOACH_ACTIVE_FIELD_ID) || fieldByName(customFields, ["smartcoach active", "smartcoach_active"])),
  };
}
...
function contactFieldValue(fields, id) {
  ...
  return clean(field && (field.value || field.fieldValue || field.field_value));
}

function fieldByName(fields, names) {
  const wanted = names.map((item) => clean(item).toLowerCase());
  const field = (Array.isArray(fields) ? fields : []).find((item) => wanted.includes(clean(item && (item.name || item.fieldName || item.key)).toLowerCase()));
  return clean(field && (field.value || field.fieldValue || field.field_value));
}
