...
function buildTrainingPlanDayProperties(plan, day, planRecord) {
  ...
  return compactProperties({
    ...
    status: dayStatusValue(day.status || "scheduled"),
...
function normalizePlanDays(days) {
  ...
  return days.map((day, index) => ({
    ...
    status: clean(day && day.status) || "scheduled",
  ...
  return {
    ...
    fields: [
      ...
      },
      {
        ...
        type: "textarea",
        required: false,
        placeholder: "Years consistently training, years of structured workouts, mileage history",
      },
      {
        key: "weeklyVolumeCapacity",
        label: "Weekly Volume Capacity",
        type: "textarea",
        required: false,
        placeholder: "Sustainable weekly mileage now, recent high weeks, long run tolerance",
      },
      {
        key: "athleteProfile",
        ...
        type: "textarea",
        required: false,
        placeholder: "Speed-based, aerobic/rhythm runner, mixed profile, race tactics, strengths",
      },
      {
        key: "recoveryAbility",
        ...
        type: "textarea",
        required: false,
        placeholder: "Sleep, school/work stress, nutrition, heat tolerance, hard-day spacing",
      },
      {
        key: "availableTrainingTime",
        label: "Available Training Time",
        type: "textarea",
        required: false,
        placeholder: "Days per week, practice length, track/hills/weights access, travel, other sports",
      },
      {
        key: "adaptationRate",
        ...
        type: "textarea",
        required: false,
        placeholder: "Responds quickly, needs gradual progression, injury risk when volume rises",
      },
      {
        key: "trainingLimits",
...
function currentSeason() {
  ...
  if (month === 12 || month === 1) return { season: "Winter", year };
  if (month >= 2 && month <= 5) return { season: "Spring", year };
  if (month >= 6 && month <= 7) return { season: "Summer", year };
