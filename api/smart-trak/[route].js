  ...
  return (Array.isArray(items) ? items : []).map((item) => {
    ...
    const athletes = (Array.isArray(source.athletes) ? source.athletes : []).map((athlete, index) => {
      ...
      return {
        ...
        startBlockIndex: Math.max(0, Number.parseInt(row.startBlockIndex, 10) || 0),
        ...
        results: (Array.isArray(row.results) ? row.results : []).map((result) => ({
          ...
          leftReps: result && result.leftReps != null ? Math.min(1000, Math.max(0, Number.parseInt(result.leftReps, 10) || 0)) : null,
          rightReps: result && result.rightReps != null ? Math.min(1000, Math.max(0, Number.parseInt(result.rightReps, 10) || 0)) : null,
    ...
    return {
      ...
      startBlockIndices: Array.isArray(source.startBlockIndices) ? [0, 1].map((index) => Math.max(0, Number.parseInt(source.startBlockIndices[index], 10) || 0)) : [0, 0],
...
function normalizePowerTrakExercises(items, blockLabel) {
  ...
  return (Array.isArray(items) ? items : []).map((item, index) => {
    ...
    return {
      ...
      executionMode: ["both", "each_side", "alternating", "left_only", "right_only"].includes(source.executionMode) ? source.executionMode : "both",
